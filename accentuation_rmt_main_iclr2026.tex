...
\red{Our main analytical tool is a collection of deterministic equivalents (DEs) relationships applied to the quadratic forms $N$ and $D$. These relations replace resolvents of the random sample covariance $\hat\Sigma$ with those of the population covariance $\Sigma$. Finite-sample randomness is summarized at leading order by an effective regularization parameter $\kappa$, determined by the self-consistent Silverstein equation}
\begin{equation}
...
of $\Sigma_{F}$.
\end{prop}
Note for this feature-space DE, the proportional aspect ratio is $p/n$ and every $\mathrm{df}_{r,q}$ is evaluated using the spectrum of $\Sigma_F$; this replaces the pixel-space ratio $d/n$ and spectrum of $\Sigma$.
