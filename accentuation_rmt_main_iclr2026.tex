...
With these structure we can already construct cases where
...
In practice, people often do cross validated regularization selection e.g. RidgeCV. 
We note that, in ideal case, the RidgeCV is optimizing regularization for in distribution $E_{gen}$. How does that affects or constrain the accentuation and control error?
...
First we show that in our simplistic setting, there is always a regularization strength that minimizes accentuation error, and in general case that regularization strength is not identical to the one selected for generalization, and this difference is also what causes the failure of control even regularization has been optimized for prediction. 
...
\end{equation}
Note we didn't prove the uniqueness or existence of such stationary
...
% When we select regularization parameter by minimizing generalization
% error $E_{gen,DE}$ similar to RidgeCV, at this stationary point, accentuation error reads
% \begin{align}
...
\todo{Exactly Answer the question from the title: when is control esp. bad? discuss it}
