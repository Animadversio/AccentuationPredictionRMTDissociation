...
{\color{red}
Let $S:={\bstar}^{\top}\Sigma\bstar$ denote the natural teacher-signal variance, $N:={\bhat}^{\top}\bstar$ the teacher--student alignment, and $D:={\bhat}^{\top}\bhat$ the squared student norm. Under the variance-matched accentuation protocol in Eq.~\ref{eq:variance-matching},
\begin{equation}
...
\end{equation}
Thus the three accentuation metrics depend on the same two quadratic forms. Their apparent asymmetry comes from the orientation of the ratio: slope and normalized MSE use $N/D$, whereas $R_{acc}^2$ is naturally expressed through $D/N-1$. Keeping the formulas in terms of $N$ and $D$ makes this geometry explicit.
}
...
\red{\textbf{Tentative example: an asymptotically exact separation.}}
{\color{red}
...
{\color{red}
\begin{remark}[Ratio of deterministic equivalents]
The accentuation metrics are nonlinear functions of the random pair $(N,D)$. In the main text we use the leading approximation obtained by replacing each quadratic form with its deterministic equivalent, for example $D/N\approx\mu_D/\mu_N$. This approximation is accurate when $N$ and $D$ concentrate jointly in the proportional high-dimensional limit, but it is not an exact identity for finite $n,d$. The appendix develops a perturbative ratio correction and a Gaussian distributional-DE approximation for regimes in which the residual variance of $(N,D)$ is non-negligible; see App.~\ref{sec:app-ratio-approximations} and App.~\ref{sec:app-ratio-corrections}.
\end{remark}
}

\paragraph{Deterministic-equivalent toolkit.}
\red{Our main analytical tool is a collection of deterministic equivalents (DEs) for the quadratic forms defining $N$ and $D$. These relations replace resolvents of the random sample covariance $\hat\Sigma$ with functions of the population covariance $\Sigma$. Finite-sample randomness is summarized at leading order by an effective regularization parameter $\kappa$, determined by the self-consistent Silverstein equation}
\begin{equation}
...
\red{For compactness in the following DE identities only, define the relative accentuation response error $z_{acc}:=D/N-1$.}
\begin{cor}\label{cor:pixel-zacc-de}
Equivalent forms of the relative accentuation response error are
\begin{align}
