...
Later, we utilize eigendecomposition of the population covariance $\Sigma=\sum_ks_k\uvec_k\uvec_k^\top$, and express the teacher weight on the PC basis $\bstar=\sum_k b_k \uvec_k$. 
...
\end{equation}
Thus the fitted weight is not only a predictor: it also determines the direction along which the predictor is evaluated.\footnote{This is an idealized version of deep-network feature accentuation, which may additionally use preconditioning, augmentation, and image priors.\cite{} }
...
\end{equation}
In \red{Appendix~\ref{}}, we will discuss the generalization to non-centered seed image and multi seed images case.%\ref{sec:app-evaluation-distributions}
...
The remaining question is no longer whether such a dissociation is possible, but when finite-sample learning, response noise, and a feature map produce it, and whether regularization fixes it; the following sections answer that question quantitatively.
...
\todo{COMMENT ON THIS, when is it the most vulnerable}
...
\todo{Answer given a teacher what kind of feature map is the most dangerous?}
