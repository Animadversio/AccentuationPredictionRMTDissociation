...
\footnotetext{This is a simplified version of the actual procedure used for deep networks, which typically involve additional pre-conditioning and augmentation steps to regularize the optimization process.}
...
In the example above, we designed the weight and their differences, but how does learning or optimization arrive at such different weights? 
\red{Here we argue that noisy target, spectrum of the input distribution, and the feature map all accentuates this issue. at least in the linear case.}

Here we consider a classical set up where a linear model is learning from a linear teacher with additive noise \cite{}. % This system is very well studied, with ample beautiful papers. 
...
On a natural data distribution \red{$\x\sim\mathcal{N}(0,\Sigma)$ in $d$-dim}\todo{Gaussian assumption can be alleviated}. 
The teacher is linear model with weights $\bstar\in\R^d$. Where the student observe the response with additive i.i.d. Gaussian noise $\epsilon\sim\mathcal{N}(0,1)$ at level $\sigma$
\[
y=\x^{\top}\bstar+\sigma\epsilon
\]
The student model is bias-free linear model with weight $\bhat$,
which we will extend to effective weights in linear feature case in Sec.~\ref{}. 
\[
f(\x)=\x^{\top}\bhat
\]
The student see $n$ training samples $\{\x_i,y_i\}$. We stack the training samples into a matrix $X\in \R^{n\times d}$, oberved noisy responses into $\mathbf y\in \R^n$ and the realized noise $\eta\in\R^n$, and the empirical data covariance $\hat\Sigma=\frac{1}{n}X^\top X$.  %observation
...
\end{equation}
For accentuation and control, however, the student model propose inputs based on its predicted weights. As a result, the model is evaluated on a self generated distribution
\[
\x_{0}+\alpha\bhat
\]
...
\[
\mathrm{Var}[\alpha](\bhat^{\top}\bhat)^2=\beta^{\star\top}\Sigma\bstar
\]
In Appendix \ref{}, we will discuss the generalization to non-centered seed image and multi seed images case. 
...
\hline 
Peer review & $0$ & $\mathrm{Var}[\alpha]\beta^{\prime}\beta^{\prime}{}^{\top}$ & $\mathrm{Var}[\alpha^{\prime}]\big(\beta^{\prime}{}^{\top}(\bhat-\bstar)\big)^{2}$ & $1-\big(\frac{{\beta^\prime}^{\top}\bhat}{{\beta^\prime}^{\top}\bstar}-1\big)^{2}$ & $\frac{\beta^{\prime}{}^{\top}\bstar}{\beta^{\prime}{}^{\top}\bhat}$ \tabularnewline % & $1$ or $nan$
\hline 
\end{tabular}
\par\end{center}
...
One key structural observations is, all three metrics (MSE,R2,Slope) of accentuation and control error are governed by a scalar summary statistics, 
a ratio between two scalars, where $N:=\bhat^{\top}\bstar$ is the dot-product of teacher and student weight and $D:=\bhat^{\top}\bhat$ is the squared norm of student weight. 
\begin{equation}
% \frac{N}{D}=\frac{\bhat^{\top}\bstar}{\bhat^{\top}\bhat}
E_{acc}/S=(1-\frac{N}{D})^2,\quad R^2=1-(\frac DN-1)^2,\quad slope=\frac ND \label{eq:acc_metric_ND_depen}
\end{equation}
Thus, to study accentuation error, the central aim is to estimation of this ratio and various nonlinear functions of it (MSE, R2, slope). 
...
This is justified when $N$ and $D$ concentrates around their mean (App.~\ref{}), which is the case at the proportional high-dimensional limit. Our claims are supported by numerical experiments in normal high-dim regime, showing the leading order approximation is often enough. 

However, we developed other higher order ways to estimate this ratio, e.g. adding 2nd order variance correction term, and estimating the nonlinear function with Monte Carlo of $\bhat$. These corrections are useful when $d,n$ are not high enough and $N,D$ has significant variance. For details, see App. \ref{}.
...
\end{equation}
where the classic $\mathrm{df}_{1}\equiv\mathrm{df}_{1,1}$ and $\mathrm{df}_{1}\equiv\mathrm{df}_{2,2}$. 
The full notations, 1-point and 2-point DE formula and their technical meanings are provided in App.~\ref{}. 
...
\end{equation}
\red{where $\alpha=n\lambda$ is more commonly used in packages like \texttt{sklearn}. }
...
\mu_{N}=\mathbb{E}_{X,\eta}\Big[N\Big] & \asymp\beta^{*\top}(\Sigma+\kappa I_{d})^{-1}\Sigma\bstar =B_{1,1}(\kappa)\\
\mu_{D}=\mathbb{E}_{X,\eta}\Big[D\Big] & \asymp\beta^{*\top}(\Sigma+\kappa I_{d})^{-2}\Sigma^{2}\bstar+\frac{\mathrm{Tr}\Big[\Sigma(\Sigma+\kappa I_{d})^{-2}\Big]}{n-\mathrm{df}_{2}(\kappa)}\Big(\sigma^{2}+\kappa^{2}\beta^{*\top}(\Sigma+\kappa I_{d})^{-2}\Sigma\bstar\Big)\notag\\
...
We consider a linear feature map $F\in\R^{p \times d}$. Teacher remains the same, while student is a linear function on the feature space with weights $w\in\R^p$
\[
f(\x)=w^{\top}F\x
\]
so the effective weights in input space i.e. gradient is $\bhat=F^{\top}w=\nabla_\x f$
...
\begin{cor}[Distinct summary statistics of prediction and control]
\red{On the leading DE level}, generalization error $E_{gen,DE}$ has summary
statistics $\Sigma_{F},F\Sigma\bstar,\bstar^{\top}\Sigma\bstar,\kappa$,
while it does not explicitly depend on $\Sigma,F,\bstar$. %condition on these summary stats. 
In contrast, control and accentuation error have additional dependency on $FF^{\top}$ and $F\bstar$. This dependency structure is summarized in Fig.~\ref{fig:summary_stats_dep_graph}. 
\end{cor}
...
Input spectrum -- for white, non structured data, this dissociation doen't exist. 

Noisy target -- 

Feature map -- Given these conditions, some feature maps are more vulnerable than others. 
...
under such case we have $Var[\alpha]=\frac{\beta^{*\top}\Sigma\bstar}{(\bhat^{\top}\bhat)^{2}}$
