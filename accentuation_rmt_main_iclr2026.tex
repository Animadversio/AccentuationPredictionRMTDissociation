...
\red{We summarize the closed-loop experiments of \citet{prince2026parametric}, which motivate the distinction between prediction and control (Fig.~\ref{fig:Setup_NeuroControl_Results}).}
\paragraph{Experimental setup.}
\red{For each of $25$ visual-cortical sites in five macaques, ten DNN feature spaces were paired with cross-validated ridge readouts fit to responses to natural images. Each fitted model then generated graded image sweeps from natural-image seeds, targeting $11$ predicted response levels; the $27{,}500$ synthesized stimuli were presented back to the animals. This tests the model on inputs chosen by its own encoding axis rather than on another held-out sample from the natural-image distribution.}
