...
\begin{proposition}[Feature-space deterministic equivalents]
\label{prop:feature-de}
Let $\kappa$ solve Eq.~\eqref{eq:kappa-fixed-point} with $\Sigma$ replaced by
$C$, and define
\begin{equation}
  \Rcal=(C+\kappa I)^{-1},
  \qquad
...
\begin{equation}
  v_0
  =\frac{\kappa^2\tstar^{\top}\Rcal C\Rcal\tstar
          +S_{\perp}+\sigma^2}
         {n-\tr(C^2\Rcal^2)}.
  \label{eq:v0}
\end{equation}
...
\begin{align}
  E_{\rm gen}^{\rm DE}
    &=S_{\perp}
      +(\bm\mu-\tstar)^{\top}C(\bm\mu-\tstar)
      +v_0\tr(C\Rcal C\Rcal),\label{eq:feature-egen-de}\\
  N_{\rm acc}^{\rm DE}
    &=\bm q^{\top}\bm\mu,\label{eq:feature-nacc-de}\\
  D_{\rm acc}^{\rm DE}
    &=\bm\mu^{\top}G\bm\mu
      +v_0\tr(G\Rcal C\Rcal).\label{eq:feature-dacc-de}
\end{align}
Equivalently, $v_0=(E_{\rm gen}^{\rm DE}+\sigma^2)/n$.
\end{proposition}
...
\begin{itemize}
  \item Explain why a ratio of deterministic means can predict typical
        ...
        denominator.
  \item Retain an approximate Gaussian distribution for the fitted feature
        coefficient:
  \begin{equation}
    \that\ \dot\sim\ \mathcal N(\bm\mu,V),
    \qquad V=v_0\Rcal C\Rcal.
...
are
\begin{align}
  \E N_{\rm acc}&=\bm q^{\top}\bm\mu,
  &\Var(N_{\rm acc})&=\bm q^{\top}V\bm q,\\
  \E D_{\rm acc}&=\bm\mu^{\top}G\bm\mu+\tr(GV),
  &\Var(D_{\rm acc})
