...
This opening list is deliberately more comprehensive than the main text.  It
serves as an index of the theoretical claims currently supported by the
appendix; the final paper may promote only a subset of them to the main body.
All statements concern the linear teacher--student model unless explicitly
marked as a nonlinear diagnostic.

\begin{enumerate}
\item \textbf{Evaluation geometry.} Generalization, self-accentuation, and
peer review are evaluations under different input covariances. Their equal-error
sets are, respectively, covariance ellipsoids, Euclidean spheres, and
peer-normal hyperplanes (Section~\ref{sec:app-evaluation-geometry}).

\item \textbf{Pixel-ridge prediction DE.} For anisotropic Gaussian design,
the generalization risk separates into ridge bias and finite-sample variance,
with the random sample covariance summarized by the effective penalty
$\kappa$ (Section~\ref{sec:app-pixel-ridge}).

\item \textbf{Pixel-ridge control DE.} The accentuation metrics reduce to two
quadratic forms $N=\bhat^\top\bstar$ and $D=\bhat^\top\bhat$.  Their leading
DEs expose a Euclidean variance term that is not determined by prediction risk.

\item \textbf{Prediction- versus control-optimal tuning.} An interior
prediction optimum and a zero-leading-bias control optimum equate different
spectral averages.  Prediction-optimal CV therefore leaves a residual
accentuation bias except in special symmetric cases
(Section~\ref{sec:app-optimal-regularization}).

\item \textbf{Feature-space projection.} The population-optimal realizable
coefficient is $w^\star=\Sigma_F^\dagger F\Sigma\bstar$, with irreducible
error $S_\perp$.  Feature-space ridge inherits the pixel-space prediction DE
with effective noise $S_\perp+\sigma^2$, but pixel-space control additionally
depends on $FF^\top$ and $F\bstar$
(Section~\ref{sec:app-feature-regression}).

\item \textbf{Prediction gauge.} Transformations preserving
$F\Sigma F^\top$ and $F\Sigma\bstar$ preserve the leading prediction risk and
DE-based CV choice while allowing control to vary.  For $\Sigma\succ0$, a
constructive family is $F\mapsto F\Sigma^{1/2}Q\Sigma^{-1/2}$ with
$Q\Sigma^{1/2}\bstar=\Sigma^{1/2}\bstar$
(Section~\ref{sec:app-gauge-fragility}).

\item \textbf{Low-variance gradient fragility.} The control variance factor
$T_F(\kappa)$ is a ridge-weighted sum of
$\|g_k\|^2/(g_k^\top\Sigma g_k)$.  Control is fragile when feature PCs that
survive regularization backpropagate into low-variance input directions.

\item \textbf{Ratio corrections.} Ratio-of-DE, second-order delta, and
Gaussian distributional approximations answer different finite-sample
questions.  The corrections matter when the mean calibration error is small
but the variance of $N/D$ is not (Section~\ref{sec:app-ratio-corrections}).

\item \textbf{Nonlinear diagnostic.} Replacing $F$ locally by the Jacobian of
a nonlinear feature map yields exact-local and neighborhood diagnostics. This
is not a nonlinear RMT theorem, but it predicts model ordering in the neuronal
control data (Section~\ref{sec:app-nonlinear-diagnostic}).

\item \textbf{RMT identities.} The final section records the one- and
two-resolvent deterministic equivalents, fixed-point relation, and spectral
recursion identities used in the proofs
(Section~\ref{sec:app-rmt-toolkit}).
\end{enumerate}
\endgroup
...
noise.
\[
y={\bstar}^{\top}\mathbf{x}+\sigma\epsilon,\quad\epsilon\sim\mathcal{N}(0,I)
\]
...
trying to estimate the weight vector $\bhat$.
...
\begin{align*}
\bhat & =(X^{\top}X+n\lambda I_{d})^{-1}X^{\top}\mathbf{y}\\
 & =(X^{\top}X+n\lambda I_{d})^{-1}X^{\top}(X\bstar+\sigma\eta)\\
 & =(X^{\top}X+n\lambda I_{d})^{-1}X^{\top}X\bstar+\sigma(X^{\top}X+n\lambda I_{d})^{-1}X^{\top}\eta\\
 & =(\hat{\Sigma}+\lambda I_{d})^{-1}\hat{\Sigma}\bstar+\sigma(\hat{\Sigma}+\lambda I_{d})^{-1}(\frac{1}{n}X^{\top}\eta)
\end{align*}
%
\end{minipage}}}
...
In our case the linear model is the neuron predictive model, $f(\mathbf{x})={\bhat}^{\top}\mathbf{x}$,
$f(\mathbf{x})$ is the predicted response, $\bhat$ is the
...
gradient direction
\[
\mathbf{x}\gets\mathbf{x}+\alpha\nabla f=\mathbf{x}+\alpha\mathbf{\bhat}
\]
 until $f(x)=y^{*}$.
...
(see derivation)
\[
\mathbf{x}^{*}=\mathbf{x}_{0}+\frac{y^{*}-{\bhat}^{\top}\mathbf{x}_{0}}{\|\mathbf{\bhat}\|^{2}}\bhat
\]
...
$\mathbf{x}=\alpha\bhat$ with $\alpha\sim p_{\alpha}$.
...
\mathrm{MSE}[p] & =\mathbb{E}_{\mathbf{x}\sim p}[\big(\hat{y}(\mathbf{x})-y^{\star}(\mathbf{x})\big)^{2}]\\
 & =\mathbb{E}_{\mathbf{x}\sim p}[\big((\bhat-\bstar)^{\top}\mathbf{x}\big)^{2}]\\
 & =(\bhat-\bstar)^{\top}\mathbb{E}_{\mathbf{x}\sim p}[\mathbf{x}\mathbf{x}^{\top}](\bhat-\bstar)\\
 & =(\bhat-\bstar)^{\top}\big(\mu_{p}\mu_{p}^{\top}+\Sigma_{p}\big)(\bhat-\bstar)
\end{align*}
\end{lem}
...
$p$ is
\begin{align*}
R^{2} & =1-\frac{\mathbb{E}[(\hat{y}-y^{\star})^{2}]}{\mathrm{Var}[y^{\star}]}\\
 & =1-\frac{\mathbb{E}_{\mathbf{x}\sim p}[\big((\bhat-\bstar)^{\top}\mathbf{x}\big)^{2}]}{\mathrm{Var}[{\bstar}^{\top}\mathbf{x}]}\\
 & =1-\frac{(\bhat-\bstar)^{\top}\big(\mu_{p}\mu_{p}^{\top}+\Sigma_{p}\big)(\bhat-\bstar)}{{\bstar}^{\top}\Sigma_{p}\bstar}\\
 & =1-\frac{\mathrm{MSE}[p]}{{\bstar}^{\top}\Sigma_{p}\bstar}
\end{align*}
\end{lem}
%
\begin{lem}
...
r & =\frac{\mathbb{E}[(\hat{y}-\bar{\hat{y}})(y^{\star}-\bar{y^{\star}})]}{\sqrt{\mathrm{Var}[\hat{y}]\mathrm{Var}[y^{\star}]}}\\
 & =\frac{\mathbb{E}[({\bhat}^{\top}(\mathbf{x}-\bar{\mathbf{x}}))({\bstar}^{\top}(\mathbf{x}-\bar{\mathbf{x}}))]}{\sqrt{\mathrm{Var}[\hat{y}]\mathrm{Var}[y^{\star}]}}\\
 & =\frac{{\bhat}^{\top}\Sigma_{p}\bstar}{\sqrt{({\bhat}^{\top}\Sigma_{p}\bhat)({\bstar}^{\top}\Sigma_{p}\bstar)}}
\end{align*}
\end{lem}
%
\begin{lem}
...
\mathrm{slope}_{y^{\star}\gets\hat{y}} & =\frac{\mathrm{Cov}[\hat{y},y^{\star}]}{\mathrm{Var}[\hat{y}]}\\
 & =\frac{\mathbb{E}[({\bhat}^{\top}(\mathbf{x}-\bar{\mathbf{x}}))({\bstar}^{\top}(\mathbf{x}-\bar{\mathbf{x}}))]}{{\bhat}^{\top}\Sigma_{p}\bhat}\\
 & =\frac{{\bhat}^{\top}\Sigma_{p}\bstar}{{\bhat}^{\top}\Sigma_{p}\bhat}
\end{align*}
\end{lem}
Remarks
\begin{itemize}
...
then Pearson $r=1$, as long as ${\bhat}^{\top}\mathbf{v}\neq0$
and ${\bstar}^{\top}\mathbf{v}\neq0$. So in a sense, when
...
procedure as above $\mathbf{x}^{\prime}=\mathbf{x}_{0}+\alpha\bhat$
(starting at $\mathbf{x}_{0}$, then move along $\bhat$ with
coefficient $\alpha$ with variance $\mathrm{Var}[\alpha]$, and $\mathbb{E}[\alpha]=0$)
\[
E_{acc}=(\bhat-\bstar)^{\top}(\mathbf{x}_{0}\mathbf{x}_{0}^{\top}+\mathrm{Var}[\alpha]\bhat{\bhat}^{\top})(\bhat-\bstar)
\]
...
reads
\[
E_{peer\ rev}=(\bhat-\bstar)^{\top}(\mathbf{x}_{0}\mathbf{x}_{0}^{\top}+\mathrm{Var}[\alpha]\bprime{\bprime}^{\top})(\bhat-\bstar)
\]
\end{enumerate}
...
\begin{align*}
E_{gen} & =(\bhat-\bstar)^{\top}\Sigma(\bhat-\bstar)\\
E_{acc} & =\mathrm{Var}[\alpha]\big({\bhat}^{\top}(\bhat-\bstar)\big)^{2}\\
E_{peer\ rev} & =\mathrm{Var}[\alpha^{\prime}]\big({\bprime}^{\top}(\bhat-\bstar)\big)^{2}
\end{align*}
...
\hline
Generalization & $0$ & $\Sigma$ & $(\bhat-\bstar)^{\top}\Sigma(\bhat-\bstar)$ & $1-\frac{(\bhat-\bstar)^{\top}\Sigma(\bhat-\bstar)}{{\bstar}^{\top}\Sigma\bstar}$ & $\frac{{\bhat}^{\top}\Sigma\bstar}{{\bhat}^{\top}\Sigma\bhat}$ & $\frac{{\bhat}^{\top}\Sigma\bstar}{\sqrt{({\bhat}^{\top}\Sigma\bhat)({\bstar}^{\top}\Sigma\bstar)}}$\tabularnewline
\hline
Accentuation & $0$ & $\mathrm{Var}[\alpha]\bhat{\bhat}^{\top}$ & $\mathrm{Var}[\alpha]\big({\bhat}^{\top}(\bhat-\bstar)\big)^{2}$ & $1-\big(\frac{{\bhat}^{\top}\bhat}{{\bhat}^{\top}\bstar}-1\big)^{2}$ & $\frac{{\bhat}^{\top}\bstar}{{\bhat}^{\top}\bhat}$ & $1$ or $nan$\tabularnewline
\hline
Peer review & $0$ & $\mathrm{Var}[\alpha]\bprime{\bprime}^{\top}$ & $\mathrm{Var}[\alpha^{\prime}]\big({\bprime}^{\top}(\bhat-\bstar)\big)^{2}$ & $1-\big(\frac{{\bprime}^{\top}\bhat}{{\bprime}^{\top}\bstar}-1\big)^{2}$ & $\frac{{\bprime}^{\top}\bstar}{{\bprime}^{\top}\bhat}$ & $1$ or $nan$\tabularnewline
\hline
\end{tabular}
\par\end{centering}
...
for Pearson, nan happens when ${\bhat}^{\top}\bstar=0$
for accentuation; when ${\bhat}^{\top}\bprime=0$ or ${\bprime}^{\top}\bhat=0$
...
\begin{align*}
\operatorname{Var}_{\mathbf{x}^{\prime}=\mathbf{x}_{0}+\alpha\bhat}[\hat{y}(\mathbf{x})] & =\operatorname{Var}_{x\sim\mathcal{N}(0,\Sigma)}[y^{\star}]\\
\operatorname{Var}_{\mathbf{x}^{\prime}=\mathbf{x}_{0}+\alpha\bhat}[{\bhat}^{\top}\mathbf{x}^{\prime}] & =\operatorname{Var}_{x\sim\mathcal{N}(0,\Sigma)}[{\bstar}^{\top}\mathbf{x}]\\
\mathrm{Var}[\alpha]({\bhat}^{\top}\bhat)^{2} & ={\bstar}^{\top}\Sigma\bstar\\
\mathrm{Var}[\alpha] & =\frac{{\bstar}^{\top}\Sigma\bstar}{({\bhat}^{\top}\bhat)^{2}}
\end{align*}
...
\begin{align*}
E_{acc} & =\mathrm{Var}[\alpha]\big({\bhat}^{\top}(\bhat-\bstar)\big)^{2}\\
 & =\frac{{\bstar}^{\top}\Sigma\bstar}{({\bhat}^{\top}\bhat)^{2}}\big({\bhat}^{\top}(\bhat-\bstar)\big)^{2}\\
 & =({\bstar}^{\top}\Sigma\bstar)\big(1-\frac{{\bhat}^{\top}\bstar}{{\bhat}^{\top}\bhat}\big)^{2}
\end{align*}

Note the variance of the true model on the accentuation path will
be
\[
\operatorname{Var}_{\mathbf{x}^{\prime}=\mathbf{x}_{0}+\alpha\bhat}[{\bstar}^{\top}\mathbf{x}^{\prime}]=({\bstar}^{\top}\bhat)^{2}\mathrm{Var}[\alpha]
\]
\begin{align*}
R_{acc}^{2} & =1-\frac{E_{acc}}{\mathrm{Var}[{\bstar}^{\top}\mathbf{x}^{\prime}]}\\
 & =1-\frac{\mathrm{Var}[\alpha]\big({\bhat}^{\top}(\bhat-\bstar)\big)^{2}}{\mathrm{Var}[\alpha]({\bstar}^{\top}\bhat)^{2}}\\
 & =1-\frac{\big({\bhat}^{\top}(\bhat-\bstar)\big)^{2}}{({\bstar}^{\top}\bhat)^{2}}\\
 & =1-\Big(1-\frac{{\bhat}^{\top}\bhat}{{\bstar}^{\top}\bhat}\Big)^{2}
\end{align*}

The slope between true activation and predicted activation will be
(noiseless case)
\begin{align*}
 & \mathrm{slope}_{x^{\prime}=\mathbf{x}_{0}+\alpha\bhat}({\bstar}^{\top}\mathbf{x}^{\prime},{\bhat}^{\top}\mathbf{x}^{\prime})\\
= & \mathrm{slope}({\bstar}^{\top}(\mathbf{x}_{0}+\alpha\bhat),{\bhat}^{\top}(\mathbf{x}_{0}+\alpha\bhat))\\
= & \mathrm{slope}({\bstar}^{\top}\bhat,{\bhat}^{\top}\bhat)\\
= & \frac{{\bstar}^{\top}\bhat}{{\bhat}^{\top}\bhat}
\end{align*}
...
\begin{align*}
E_{peer\ rev} & =\mathrm{Var}[\alpha^{\prime}]\big({\bprime}^{\top}(\bhat-\bstar)\big)^{2}\\
 & =\frac{{\bstar}^{\top}\Sigma\bstar}{({\bprime}^{\top}\bprime)^{2}}\big({\bprime}^{\top}(\bhat-\bstar)\big)^{2}\\
 & ={\bstar}^{\top}\Sigma\bstar\big(\frac{{\bprime}^{\top}\bhat}{{\bprime}^{\top}\bprime}-\frac{{\bprime}^{\top}\bstar}{{\bprime}^{\top}\bprime}\big)^{2}
\end{align*}

The true neural variance is
\[
\operatorname{Var}_{\mathbf{x}^{\prime}=\mathbf{x}_{0}+\alpha^{\prime}\bprime}[{\bstar}^{\top}\mathbf{x}^{\prime}]=({\bstar}^{\top}\bprime)^{2}\mathrm{Var}[\alpha^{\prime}]
\]

the corresponding peer reviewed R2 is
\begin{align*}
R_{peer\ review}^{2}= & 1-\frac{E_{peer\ rev}}{\operatorname{Var}_{\mathbf{x}^{\prime}=\mathbf{x}_{0}+\alpha^{\prime}\bprime}[{\bstar}^{\top}\mathbf{x}^{\prime}]}\\
= & 1-\frac{\mathrm{Var}[\alpha^{\prime}]\big({\bprime}^{\top}(\bhat-\bstar)\big)^{2}}{\mathrm{Var}[\alpha^{\prime}]({\bstar}^{\top}\bprime)^{2}}\\
= & 1-\Big(\frac{{\bprime}^{\top}(\bstar-\bhat)}{{\bstar}^{\top}\bprime}\Big)^{2}\\
= & 1-\Big(1-\frac{{\bhat}^{\top}\bprime}{{\bstar}^{\top}\bprime}\Big)^{2}
\end{align*}

The slope reads between the model predicted and true neural response
reads,
\begin{align*}
 & \mathrm{slope}_{\mathbf{x}^{\prime}=\mathbf{x}_{0}+\alpha^{\prime}\bprime}({\bstar}^{\top}\mathbf{x}^{\prime},{\bhat}^{\top}\mathbf{x}^{\prime})\\
= & \mathrm{slope}({\bstar}^{\top}(\mathbf{x}_{0}+\alpha^{\prime}\bprime),{\bhat}^{\top}(\mathbf{x}_{0}+\alpha^{\prime}\bprime))\\
= & \mathrm{slope}({\bstar}^{\top}\bprime,{\bhat}^{\top}\bprime)\\
= & \frac{{\bstar}^{\top}\bprime}{{\bhat}^{\top}\bprime}
\end{align*}

For comparison, on a general centered evaluation distribution with covariance
$\Sigma_p$, Pearson correlation is
\begin{align*}
\operatorname{corr}(y_{true},y_{pred})
&=\operatorname{corr}({\bstar}^{\top}x,{\bhat}^{\top}x)\\
&=\frac{{\bhat}^{\top}\Sigma_p\bstar}
{\sqrt{{\bhat}^{\top}\Sigma_p\bhat}\sqrt{{\bstar}^{\top}\Sigma_p\bstar}}.
\end{align*}
On a noiseless single peer-review path, $\Sigma_p$ has rank one, so the
correlation is $+1$, $-1$, or undefined and carries no calibration information.
...
The three evaluation distributions induce qualitatively different
geometries in coefficient space.  For a fixed positive-definite $\Sigma$,
an equal-generalization-error set is the ellipsoid
\begin{equation}
  (\bhat-\bstar)^\top\Sigma(\bhat-\bstar)=c.
  \label{eq:iso-generalization-ellipsoid}
\end{equation}
Its semiaxis along a population eigenvector with eigenvalue $s_k$ is
proportional to $s_k^{-1/2}$.  Prediction therefore tolerates much larger
Euclidean coefficient errors in low-variance directions.

Accentuation depends instead on
\begin{equation}
  z_{acc}=\frac{\bhat^\top\bhat}{\bhat^\top\bstar}-1.
\end{equation}
For a fixed finite value $z$ with $\bhat^\top\bstar\neq0$, let $a=1+z$.
Completing the square in
$\bhat^\top\bhat=a\bhat^\top\bstar$ gives
\begin{equation}
  \left\|\bhat-\frac{a}{2}\bstar\right\|^2
  =\frac{a^2}{4}\|\bstar\|^2.
  \label{eq:iso-accentuation-sphere}
\end{equation}
Thus each equal-$z_{acc}$ set is a sphere whose diameter has endpoints
$0$ and $a\bstar$ (apart from points where the defining ratio is singular).
In particular, perfect leading-order calibration, $z_{acc}=0$, does not
require $\bhat=\bstar$: it is the sphere with diameter $[0,\bstar]$.

For a fixed peer direction $\bprime$, equal peer-review response error
satisfies
\begin{equation}
  {\bprime}^\top\!\left(\bhat-a_{peer}\bstar\right)=0,
  \qquad
  a_{peer}:=\frac{{\bprime}^\top\bhat}{{\bprime}^\top\bstar},
  \label{eq:iso-peer-hyperplane}
\end{equation}
and is therefore a hyperplane normal to $\bprime$.  Ellipsoids, spheres,
and peer-normal hyperplanes expose different components of the same
coefficient error; no scalar prediction score determines the other two.
\endgroup



\section{Pixel-space ridge deterministic equivalents and proofs}\label{sec:app-pixel-ridge}

\begingroup\color{red}
\begin{proposition}[Pixel-space ridge deterministic equivalents]
\label{prop:app-pixel-ridge-de}
Under the Gaussian design and proportional-limit assumptions of the main
text, let $\kappa$ solve the ridge fixed-point equation.  Then
\begin{align}
E_{gen,DE}
&=\frac{n\bigl(\kappa^2B_{1,2}(\kappa)+\sigma^2\bigr)}
        {n-\mathrm{df}_{2}(\kappa)}-\sigma^2,
...
\mu_D
&=B_{2,2}(\kappa)
 +\frac{\mathrm{df}_{1,2}(\kappa)}{n}
  \bigl(E_{gen,DE}+\sigma^2\bigr).
\label{eq:app-pixel-D-summary}
\end{align}
Consequently, the leading accentuation metrics follow by substituting
$N\approx\mu_N$ and $D\approx\mu_D$ into
Eq.~\ref{eq:acc_metric_ND_depen}.  Prediction and accentuation share the
same fitted estimator but apply different spectral weightings to its error.
\end{proposition}
\endgroup

\subsection{Generalization risk and its deterministic equivalent}
...
\[
\bhat=(\hat{\Sigma}+\lambda I_{d})^{-1}\hat{\Sigma}\bstar+\sigma(\hat{\Sigma}+\lambda I_{d})^{-1}(\frac{1}{n}X^{\top}\eta)
\]
\[
\bhat-\bstar=-\lambda(\hat{\Sigma}+\lambda I_{d})^{-1}\bstar+\sigma(\hat{\Sigma}+\lambda I_{d})^{-1}(\frac{1}{n}X^{\top}\eta)
\]
\begin{align*}
E_{gen} & =(\bhat-\bstar)^{\top}\Sigma(\bhat-\bstar)\\
\mathbb{E}_{\eta}[E_{gen}] & =\lambda^{2}{\bstar}^{\top}(\hat{\Sigma}+\lambda I_{d})^{-1}\Sigma(\hat{\Sigma}+\lambda I_{d})^{-1}\bstar\\
 ...
 & =\lambda^{2}{\bstar}^{\top}(\hat{\Sigma}+\lambda I_{d})^{-1}\Sigma(\hat{\Sigma}+\lambda I_{d})^{-1}\bstar\\
...
\subsection{Accentuation numerator and denominator}

From Tab. \ref{tab:theory-Evaluation-metrics}, for accentuation and
control error, the central quantity is a ratio, i.e. slope
\[
\mathrm{slope}=\frac{{\bhat}^{\top}\bstar}{{\bhat}^{\top}\bhat}
\]

Where the other metrics (MSE, R2) can be expressed with the slope
\begin{align*}
R^{2} & =1-\big(\frac{{\bhat}^{\top}\bhat}{{\bhat}^{\top}\bstar}-1\big)^{2}\\
 & =1-(\frac{1}{\mathrm{slope}}-1)^{2}
\end{align*}
\begin{align*}
E_{acc} & =({\bstar}^{\top}\Sigma\bstar)\big(1-\frac{{\bhat}^{\top}\bstar}{{\bhat}^{\top}\bhat}\big)^{2}\\
 & =({\bstar}^{\top}\Sigma\bstar)(\mathrm{slope}-1)^{2}
\end{align*}

Thus the key quantities to analyze is the numerator ${\bhat}^{\top}\bstar$
and denominator ${\bhat}^{\top}\bhat$.

Abbreviating $N:={\bhat}^{\top}\bstar$, and $D:={\bhat}^{\top}\bhat$
\[
...
\end{align*}
\[
E_{acc}=({\bstar}^{\top}\Sigma\bstar)(\frac{N}{D}-1)^{2}
\]




\subsection{Leading deterministic equivalents for $N$ and $D$}
...
\bstar & =\sum_{k=1}^{d}b_{k}\mathbf{u}_{k}
\end{align*}
...
\[
N:={\bstar}^{\top}\bhat={\bstar}^{\top}(\hat{\Sigma}+\lambda I_{d})^{-1}\hat{\Sigma}\bstar+\sigma{\bstar}^{\top}(\hat{\Sigma}+\lambda I_{d})^{-1}(\frac{1}{n}X^{\top}\eta)
\]

The expectation and the deterministic equivalence can be written
\begin{align*}
\mathbb{E}_{X,\eta}\Big[{\bhat}^{\top}\bstar\Big] & =\mathbb{E}_{X}\Big[{\bstar}^{\top}(\hat{\Sigma}+\lambda I_{d})^{-1}\hat{\Sigma}\bstar\Big]\\
 & \asymp{\bstar}^{\top}(\Sigma+\kappa(\lambda)I_{d})^{-1}\Sigma\bstar
\end{align*}

Thus
\begin{equation}
\mu_{N}=\mathbb{E}_{X,\eta}\Big[N\Big]\asymp{\bstar}^{\top}(\Sigma+\kappa(\lambda)I_{d})^{-1}\Sigma\bstar\label{eq:mean_numerator_dotprod}
\end{equation}

\textbf{Interpretations}
\begin{itemize}
\item The inner product expectation is non-negative $\mu_{N}\geq0$. The
only case where it's 0, is when $\Sigma\bstar=0$, i.e. teacher
...
\[
D:={\bhat}^{\top}\bhat={\bstar}^{\top}\hat{\Sigma}^{2}(\hat{\Sigma}+\lambda I_{d})^{-2}\bstar+2\sigma{\bstar}^{\top}\hat{\Sigma}(\hat{\Sigma}+\lambda I_{d})^{-2}(\frac{1}{n}X^{\top}\eta)+\sigma^{2}(\frac{1}{n}X^{\top}\eta)^{\top}(\hat{\Sigma}+\lambda I_{d})^{-2}(\frac{1}{n}X^{\top}\eta)
\]

The expectation and the deterministic equivalence can be written
\begin{align*}
\mathbb{E}_{X,\eta}\Big[{\bhat}^{\top}\bhat\Big] & =\mathbb{E}_{X}\Big[{\bstar}^{\top}\hat{\Sigma}^{2}(\hat{\Sigma}+\lambda I_{d})^{-2}\bstar\Big]+\mathbb{E}_{X,\eta}\Big[\sigma^{2}(\frac{1}{n}X^{\top}\eta)^{\top}(\hat{\Sigma}+\lambda I_{d})^{-2}(\frac{1}{n}X^{\top}\eta)\Big]\\
 & =\mathbb{E}_{X}\Big[{\bstar}^{\top}\hat{\Sigma}^{2}(\hat{\Sigma}+\lambda I_{d})^{-2}\bstar\Big]+\sigma^{2}\mathbb{E}_{X,\eta}\mathrm{Tr}\Big[\frac{1}{n}(\hat{\Sigma}+\lambda I_{d})^{-2}(\frac{1}{n}X^{\top}\eta\eta^{\top}X)\Big]\\
 & =\mathbb{E}_{X}\Big[{\bstar}^{\top}\hat{\Sigma}^{2}(\hat{\Sigma}+\lambda I_{d})^{-2}\bstar\Big]+\frac{\sigma^{2}}{n}\mathbb{E}_{X}\mathrm{Tr}\Big[(\hat{\Sigma}+\lambda I_{d})^{-2}\hat{\Sigma}\Big]
\end{align*}

The DE relationship are
\begin{align*}
\mathbb{E}_{X,\eta}\Big[D\Big] & =\mathbb{E}_{X}\Big[{\bstar}^{\top}\hat{\Sigma}^{2}(\hat{\Sigma}+\lambda I_{d})^{-2}\bstar\Big]+\frac{\sigma^{2}}{n}\mathbb{E}_{X}\mathrm{Tr}\Big[(\hat{\Sigma}+\lambda I_{d})^{-2}\hat{\Sigma}\Big]\\
 & \asymp{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma^{2}\bstar+\frac{\kappa^{2}\mbox{Tr}\big[\Sigma(\Sigma+\kappa I_{d})^{-2}\big]}{n-\mathrm{df}_{2}(\kappa)}{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma\bstar\\
 & \quad+\frac{\sigma^{2}}{n}\frac{n}{n-\mathrm{df}_{2}(\kappa)}\mathrm{Tr}\Big[\Sigma(\Sigma+\kappa I_{d})^{-2}\Big]\\
 & \asymp{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma^{2}\bstar+\frac{\mathrm{Tr}\Big[\Sigma(\Sigma+\kappa I_{d})^{-2}\Big]}{n-\mathrm{df}_{2}(\kappa)}\Big(\sigma^{2}+\kappa^{2}{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma\bstar\Big)
\end{align*}

Thus
\begin{equation}
\mu_{D}\asymp{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma^{2}\bstar+\frac{\mathrm{Tr}\Big[\Sigma(\Sigma+\kappa I_{d})^{-2}\Big]}{n-\mathrm{df}_{2}(\kappa)}\Big(\sigma^{2}+\kappa^{2}{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma\bstar\Big)\label{eq:mean_denom_sqnorm}
\end{equation}

\textbf{Remarks}
\begin{itemize}
\item $\mu_{D}\geq0$ because ${\bhat}^{\top}\bhat\geq0$.
\item There is a tight connection between $\mu_{D}$ and $E_{gen}$ on the
DE level
\begin{align*}
\mu_{D} & \asymp{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma^{2}\bstar+\frac{\mathrm{Tr}\Big[\Sigma(\Sigma+\kappa I_{d})^{-2}\Big]}{n-\mathrm{df}_{2}(\kappa)}\Big(\sigma^{2}+\kappa^{2}{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma\bstar\Big)\\
 & ={\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma^{2}\bstar+\frac{1}{n}\mathrm{Tr}\Big[\Sigma(\Sigma+\kappa I_{d})^{-2}\Big]\big(E_{gen}+\sigma^{2}\big)
\end{align*}
\end{itemize}
\noindent\begin{minipage}[t]{1\columnwidth}%
\textbf{Derivations}
...
\[
{\bstar}^{\top}\hat{\Sigma}^{2}(\hat{\Sigma}+\lambda I_{d})^{-2}\bstar\asymp{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma^{2}\bstar+\frac{\kappa^{2}\mbox{Tr}\big[\Sigma(\Sigma+\kappa I_{d})^{-2}\big]}{n-\mathrm{df}_{2}(\kappa)}{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma\bstar
\]
%
\end{minipage}
...
\[
\mathbb{E}[\mathrm{slope}_{acc}]=\mathbb{E}\Big[\frac{{\bhat}^{\top}\bstar}{{\bhat}^{\top}\bhat}\Big]\approx\frac{\mu_{N}}{\mu_{D}}=\frac{{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-1}\Sigma\bstar}{{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma^{2}\bstar+\frac{\mathrm{Tr}\Big[\Sigma(\Sigma+\kappa I_{d})^{-2}\Big]}{n-\mathrm{df}_{2}(\kappa)}\Big(\sigma^{2}+\kappa^{2}{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma\bstar\Big)}
\]
...
\begin{align*}
\frac{\mu_{N}}{\mu_{D}} & =\frac{{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-1}\Sigma\bstar}{{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma^{2}\bstar+\frac{\mathrm{Tr}\Big[\Sigma(\Sigma+\kappa I_{d})^{-2}\Big]}{n-\mathrm{df}_{2}(\kappa)}\Big(\sigma^{2}+\kappa^{2}{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-2}\Sigma\bstar\Big)}\\
...
\mathrm{MSE}_{acc} & =({\bstar}^{\top}\Sigma\bstar)\big(\frac{z}{z+1}\big)^{2}
\end{align*}


\paragraph{Interpretation:}

The preceding identity also gives a threshold for catastrophic accentuation
$R^2$.
...
\item $\sum_{k=1}^{d}\frac{s_{k}}{s_{k}+\kappa}b_{k}^{2}<\|\bstar\|^{2}$
...
\end{equation}
Equating this expression to zero proves Eq.~\ref{eq:acc_optim_reg_cond1}.
...
prediction optimum, inserting Eq.~\ref{eq:appendix-pixel-prediction-stationary}
into Eq.~\ref{eq:appendix-pixel-d-minus-n} gives
Eq.~\ref{eq:pixel-zacc-cv}.

\paragraph{Feature-space prediction optimum.}
Let $C=F\Sigma F^\top$, $G=FF^\top$,
$\tau^2=S_\perp+\sigma^2$, and use the shorthand in
Eq.~\ref{eq:feature-spectral-shorthand}. Here
...
\end{equation}
Setting this to zero proves Eq.~\ref{eq:feature-control-optimal}
...
recovering Eq.~\ref{eq:acc_optim_reg_cond1}.
\endgroup
...
\nabla_x[v_k^\top\phi(x)]$; therefore Eq.~\ref{eq:nonlinear-local-fragility}
