...
are $\bstar$, with Gaussian i.i.d. noise level $\sigma$
\[
y=\mathbf{x}^{\top}\bstar+\sigma\epsilon
\]

A student model is bias-free linear model with weight $\bhat$,
(or effective weight in linear feature case.)
\[
f(\mathbf{x})=\mathbf{x}^{\top}\bhat
\]
...
equivalence.
\begin{align*}
\mu_{N}=\mathbb{E}_{X,\eta}\Big[N\Big] & \asymp{\bstar}^{\top}(\Sigma+\kappa I_{d})^{-1}\Sigma\bstar\\
 & =B_{1,1}(\kappa)\\
...
feature space.
\[
S_{\perp}:=\mathbb{E}_{\mathbf{x}}[(\mathbf{x}^{\top}\bstar-\mathbf{x}^{\top}F^{\top}w^{\star})^{2}]={\bstar}^{\top}\Sigma\bstar-{w^\star}^{\top}\Sigma_{F}w^{\star}
\]
...
statistics $\Sigma_{F},F\Sigma\bstar,{\bstar}^{\top}\Sigma\bstar,\kappa$,
while it does not explicitly depend on $\Sigma,F,\bstar$condition
...
that leaves $F\Sigma F^{\top},F\Sigma\bstar$ unchanged.
\[
...
= & corr({\bhat}^{\top}x,{\bstar}^{\top}x)\\
= & \frac{{\bhat}^{\top}\Sigma\bstar}
{\sqrt{{\bhat}^{\top}\Sigma\bhat}\sqrt{{\bstar}^{\top}\Sigma\bstar}}.
\end{align*}
...
aligned with the ground truth $\bprime=\bstar$.
...
\[
\mathbb{E}\big[\mathrm{slope}_{\text{peer rev}}\big]=\frac{{\bstar}^{\top}\bstar}{{\bstar}^{\top}\bhat}
\]
...
$\Sigma^{1/2}\bstar=b$. $A\in\mathbb{R}^{p\times d}$ and
$b\in\mathbb{R}^{d}$.
...
\section{Heuristic extension to nonlinear feature maps}
...
\begingroup\color{red}
\section{Prediction- and accentuation-optimal ridge penalties}
