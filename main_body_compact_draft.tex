...
\begin{wrapfigure}{r}{0.5\linewidth}
    ...
    \vspace{-6pt}
\end{wrapfigure}
\paragraph{Learning as a path through these landscapes.}
Learning now has a geometric reading (Fig.~\ref{fig:geometry}). For a given training set, the ridge estimator traces a path $\bhat(\lambda)$ in weight space from the least-squares or minimum-norm solution at $\lambda\to0$ to the origin at $\lambda\to\infty$. Response noise and the random design scatter the fitted weight in a cloud around this path, and the cloud is elongated along the low-variance directions of $\Sigma$, because those are the directions the data constrain least. The deterministic equivalents of the next two sections compute where the center of this cloud sits and how much Euclidean spread it carries. Cross-validation then selects the point of the path that is tangent to the smallest reachable prediction ellipsoid, whereas control calibration asks where the path crosses the sphere $D=N$. The two points coincide only in special cases (isotropic data is one, Sec.~\ref{sec:pixel}); in general the noise cloud pushes the path off the sphere in precisely the directions the ellipsoid does not see.
...
\label{sec:nonlinear}
For a nonlinear feature map $\phi$ with Jacobian $J_\phi(x)$ and natural-image feature covariance eigenpairs $(s_k,v_k)$, replacing $F$ by the local Jacobian in Eq.~\eqref{eq:TF} gives
\begin{equation}
