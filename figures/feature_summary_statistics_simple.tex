...
\node[pstat] (h)  at (4.3,2.2) {\textbf{teacher--feature cov.}\\[-1pt]$\bm h=F\Sigma\bm\beta^\star$};
% ---------- control summaries
...
% ---------- control outputs
\node[cstat,text width=4.4cm] (N) at (9.6,-0.1)
  {\textbf{true gain on own path}\\[-1pt]$N=\bm q^\top(\Sigma_F+\kappa I)^{-1}\bm h$};
\node[cstat,text width=6.6cm] (D) at (9.6,-2.2)
  {\textbf{predicted gain on own path}\\[-1pt]
   $D=\bm h^\top(\Sigma_F+\kappa I)^{-1}G(\Sigma_F+\kappa I)^{-1}\bm h$\\[-1pt]
   $\;+\dfrac{E_{\rm gen}+\sigma^2}{n}\,\underbrace{\operatorname{tr}\!\big[G(\Sigma_F+\kappa I)^{-2}\Sigma_F\big]}_{T_F:\ \text{gradient geometry}}$};
\node[cout] (z) at (15.0,-1.1)
...
\begin{scope}[on background layer]
  ...
  \node[group,draw=controlred!60,fill=controlred!2,fit=(G)(q)(N)(D)(z)] (cg) {};
  ...
  \draw[pdep] (Egen) -- (kcv);
  \draw[kdep] (kcv.south) to[out=-90,in=20] node[pos=0.35,right,font=\scriptsize,text=fitpurple] {evaluate at $\kappa_{\rm CV}$} (N.east);
  \draw[kdep] (kcv.south) to[out=-90,in=10] (D.east);
  \draw[pdep] (SF.east) to[out=-20,in=160] (N.west);  \draw[pdep] (h) -- (N);
  \draw[pdep] (h.south east) to[out=-30,in=150] (D.north west);
  \draw[pdep] (Egen.south east) to[out=-70,in=75] ([xshift=-5mm]D.north east);
  \draw[cdep] (q) -- (N);   \draw[cdep] (G) -- (D);
  \draw[cdep] (N) -- (z);   \draw[cdep] (D) -- (z);
\end{scope}
...
\node[draw=inputgray!55,fill=black!2,rounded corners=1.4mm,text width=15.5cm,align=center,inner ysep=2.2mm,font=\small\sffamily]
  ...
  {Same $(\Sigma_F,\bm h,S,\sigma^2,n)$ $\Rightarrow$ same $E_{\rm gen}(\kappa)$ and same $\kappa_{\rm CV}$.\\
   $z_{\rm acc}$ additionally depends on $G$ and $\bm q$, which the gauge family $F\mapsto F\Sigma^{1/2}Q\Sigma^{-1/2}$ changes freely.};
\end{tikzpicture}
\end{document}
