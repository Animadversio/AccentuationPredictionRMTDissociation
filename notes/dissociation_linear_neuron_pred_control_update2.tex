...
We specifically characterized the \textquotedbl gauge invariance
group\textquotedbl{} of the feature map, which will not change the
...
\begin{lem}[Interior stationarity condition for prediction-optimal regularization]
The ridge parameter selection based on DE of generalization loss
needs to satisfy the first order condition
\begin{equation}
(n-\mathrm{df}_{2}\bigl(\kappa\bigr))\kappa B_{2,3}(\kappa)=\mathrm{df}_{2,3}(\kappa)\big(\kappa^{2}B_{1,2}(\kappa)+\sigma^{2}\big)\label{eq:pred_optim_reg_cond1}
\end{equation}
which can also be written through the generalization error at DE level.
\begin{equation}
E_{gen,DE}+\sigma^{2}=n\kappa\frac{B_{2,3}(\kappa)}{\mathrm{df}_{2,3}(\kappa)}\label{eq:pred_optim_reg_cond2}
\end{equation}

Currently we didn't prove the uniqueness or existence of such stationary
point across $\kappa$ , also not sure about the solution regarding
“physical” $\lambda\geq0$. 
\end{lem}
In contrast, the condition that makes the accentuation error optimal
is the following, 
\begin{prop}[Equivalent conditions for accentuation-optimal regularization]
On the leading DE level the condition of $\lambda$ or $\kappa$
that minimize and zeros accentuation error $E_{acc,DE}$, is $\mu_{N}=\mu_{D}$,
which makes $E_{acc}=0,R_{acc}^{2}=1,\text{slope}_{acc}=1$. There
exists $\exists\lambda\in[0,\infty)$ that satisfy this condition,
but the solution is not always unique. The condition $\kappa_{acc}^{\star}$
has the following equivalent forms 
\begin{align}
E_{gen,DE}(\kappa)+\sigma^{2} & =n\kappa\frac{B_{1,2}(\kappa)}{\mathrm{df}_{1,2}(\kappa)}\label{eq:acc_optim_reg_cond1}\\
\sigma^{2} & =\kappa\Big(n-\mathrm{df}_{1}(\kappa)\Big)\frac{B_{1,2}(\kappa)}{\mathrm{df}_{1,2}(\kappa)}\label{eq:acc_optim_reg_cond2}\\
\sigma^{2} & =n\lambda(\kappa)\frac{B_{1,2}(\kappa)}{\mathrm{df}_{1,2}(\kappa)}\label{eq:acc_optim_reg_cond3}\\
E_{gen,DE}(\kappa) & =(\frac{\kappa}{\lambda(\kappa)}-1)\sigma^{2}=\sigma^{2}\frac{\mathrm{df}_{1}(\kappa)}{n-\mathrm{df}_{1}(\kappa)}
\end{align}
\end{prop}
Note, this has the following note-worthy consequence, i.e. when optimizing
regularization strength for accentuation error (DE) the prediction
error satisfies some universal law. 
\begin{prop}[Generalization error at accentuation-optimal regularization]
This entails, at the zero-accentuation error points $\kappa_{acc}^{\star}$,
the generalization error at DE levels equates 
\begin{align*}
E_{gen,DE}(\kappa_{acc}^{\star}) & =\frac{\kappa}{\lambda(\kappa)}\sigma^{2}-\sigma^{2}\\
 & =\sigma^{2}\frac{\mathrm{df}_{1}(\kappa)}{n-\mathrm{df}_{1}(\kappa)}
\end{align*}
\noindent\begin{minipage}[t]{1\columnwidth}%
\textbf{Derivation}

From the two equivalent conditions \ref{eq:acc_optim_reg_cond1} and
\ref{eq:acc_optim_reg_cond3} of zero accentuation error $\kappa_{acc}^{\star}$,
one can see 
\[
\frac{E_{gen,DE}+\sigma^{2}}{\sigma^{2}}=\frac{\kappa}{\lambda(\kappa)}
\]

then we use
\[
\lambda=\kappa(\lambda)\big(1-\frac{1}{n}\mathrm{df}_{1}(\kappa)\big)
\]

We have
\begin{align*}
\frac{E_{gen,DE}+\sigma^{2}}{\sigma^{2}} & =\frac{1}{1-\frac{1}{n}\mathrm{df}_{1}(\kappa)}\\
E_{gen,DE} & =\sigma^{2}\frac{n}{n-\mathrm{df}_{1}(\kappa)}
\end{align*}
%
\end{minipage}
\end{prop}
\begin{rem}
This result is surprising because at the ``control optimal'' point,
the generalization error no longer depend on the teacher weight explicitly,
it only depend on $\kappa$ and the function of data spectral density
$\mathrm{df}_{1}(\kappa)$. In a certain sense this is an universality
result, i.e. at this special point where $\mu_{D}=\mu_{N}$ and accentuation
error is zeroed on leading order, then the generalization error follows
such universal law just depending on input geometry, 

Further this quantity upper bounded the minimal generalization error
$E_{gen,DE}(\kappa)$ one can obtain at the same noise scale. the
real optimal $\kappa$ can not be higher than this at leading level. 
\end{rem}
With this we can say the regularization strength optimal for prediction
is nearly always not optimal for accentuation and vice versa. 
\begin{prop}[Accentuation error at generalization-optimal regularization]
...
error (RidgeCV), at this stationary point $\kappa_{gen}^{\star}$\ref{eq:pred_optim_reg_cond2},
accentuation error can be written as 
\begin{align*}
z_{acc,CV}(\kappa_{gen}^{\star})\approx\frac{\mu_{D}}{\mu_{N}}-1 & =\frac{\kappa\mathrm{df}_{1,2}(\kappa)}{B_{1,1}(\kappa)}\Big(\frac{B_{2,3}(\kappa)}{\mathrm{df}_{2,3}(\kappa)}-\frac{B_{1,2}(\kappa)}{\mathrm{df}_{1,2}(\kappa)}\Big)
\end{align*}
\end{prop}
This form can be read as proportional to the difference between the
``optimal'' regularization condition for prediction and accentuation
\ref{eq:pred_optim_reg_cond2} and \ref{eq:acc_optim_reg_cond1}. 
...
\begin{prop}
On the leading DE level, fixing $n,\sigma^{2}$, generalization error
$E_{gen,DE}$ has summary statistics $\Sigma_{F},F\Sigma\beta^{\star},\beta^{\star}{}^{\top}\Sigma\beta^{\star},\kappa$,
...
\begin{prop}
One family of transformation of $F$ that leaves prediction $E_{gen}$
and $E_{gen}$ based cross validation unchanged is the Gauge transformation
group that leaves $F\Sigma F^{\top},F\Sigma\beta^{\star}$ unchanged.
\[
...
\noindent\fbox{\begin{minipage}[t]{1\columnwidth - 2\fboxsep - 2\fboxrule}%
\textbf{Detailed Calculation notes of different metrics}

\paragraph{Accentuation error}
...
\subsection{Geometry of Iso-error surface for prediction and control }

Lets consider the geometry of iso error surface to understand their
tolerance 

\paragraph{Iso-prediction error surface}

For generalization error MSE or R2, the key quantity is the following
quadratic form, 
\[
E_{gen}=(\hat{\beta}-\beta^{\star})^{\top}\Sigma(\hat{\beta}-\beta^{\star})
\]
\[
(\hat{\beta}-\beta^{\star})^{\top}\Sigma(\hat{\beta}-\beta^{\star})=\mathrm{const}
\]

Given a fixed $\beta^{\star}$ and $\Sigma$, in the non-degenerate
case, i.e. $\Sigma$ is positive definite, the iso $E_{gen}$ hypersurface
is an ellipsoid, where the deviation on each eigen direction $\mathbf{u}_{k}$
is proportional to the square root of inverse eigenvalue $1/\sqrt{s_{k}}$. 

Thus the iso-error surface is a ellipsoid, ellongated extremely in
small eigen dimensions. 

\paragraph{Iso-control error surface}

For accentuation and control error, MSE, R2 and slope all depend on
a single ratio 
\[
z=\frac{D}{N}-1=\frac{\hat{\beta}^{\top}\hat{\beta}}{\hat{\beta}^{\top}\beta^{\star}}-1
\]

where 
\[
\frac{E_{acc}}{S}=\left(1-\frac{N}{D}\right)^{2},\qquad R_{acc}^{2}=1-\left(\frac{D}{N}-1\right)^{2},\qquad\mathrm{slope}_{acc}=\frac{N}{D}.
\]

i.e.
\[
E=(\frac{z}{z+1})^{2}\qquad R^{2}=1-z^{2}\qquad slope=\frac{1}{z+1}
\]

and their optimal values are obtained at $z=0$. 

consider a fixed $z$ value
\[
\frac{\hat{\beta}^{\top}\hat{\beta}}{\hat{\beta}^{\top}\beta^{\star}}-1=z_{const}
\]

thus
\begin{align*}
\hat{\beta}^{\top}\hat{\beta} & =(1+z_{const})\hat{\beta}^{\top}\beta^{\star}\\
\hat{\beta}^{\top}\Big(\hat{\beta}-(1+z_{const})\beta^{\star}\Big) & =0
\end{align*}

Geometrically basically 
\[
\hat{\beta}\perp\hat{\beta}-(1+z_{const})\beta^{\star}
\]

First consider a special case where $z_{const}=0$ i.e. optimal, then
$\hat{\beta}\perp\hat{\beta}-\beta^{\star}$, and by definition $\hat{\beta}^{\top}\hat{\beta}=\hat{\beta}^{\top}\beta^{\star}$,
thus 
\begin{align*}
\|\hat{\beta}\|^{2}+\|\hat{\beta}-\beta^{\star}\|^{2} & =\hat{\beta}^{\top}\hat{\beta}+\hat{\beta}^{\top}\hat{\beta}-2\hat{\beta}^{\top}\beta^{\star}+\beta^{\star^{\top}}\beta^{\star}\\
 & =\|\beta^{\star}\|^{2}
\end{align*}

\[
\|\hat{\beta}\|^{2}+\|\hat{\beta}-\beta^{\star}\|^{2}=\|\beta^{\star}\|^{2}
\]

This gives us a clear geometric picture, i.e. $\hat{\beta}$ lives
on a sphere shell with diameter spanned by $0$ and $\beta^{\star}$. 

More generally, for other $z_{const}$, one can write 
\begin{align*}
\|\hat{\beta}\|^{2}+\|\hat{\beta}-(1+z_{const})\beta^{\star}\|^{2} & =\hat{\beta}^{\top}\hat{\beta}+\hat{\beta}^{\top}\hat{\beta}-2(1+z_{const})\hat{\beta}^{\top}\beta^{\star}+(1+z_{const})^{2}\|\beta^{\star}\|^{2}\\
\|\hat{\beta}\|^{2}+\|\hat{\beta}-(1+z_{const})\beta^{\star}\|^{2} & =(1+z_{const})^{2}\|\beta^{\star}\|^{2}
\end{align*}

Thus we can write
\[
\|\hat{\beta}\|^{2}+\|\hat{\beta}-(1+z_{const})\beta^{\star}\|^{2}=\|(1+z_{const})\beta^{\star}\|^{2}
\]

So $\hat{\beta}$ lives on a sphere shell spanned by $0$ and $(1+z_{const})\beta^{\star}$. 

\paragraph{Iso-peer review error surface}

Extending this to peer review 
\[
\frac{\beta^{\prime}{}^{\top}\hat{\beta}}{\beta^{\prime}{}^{\top}\beta^{\star}}=z_{const}^{\prime}+1
\]

then 
\begin{align*}
\beta^{\prime}{}^{\top}\hat{\beta} & =(z_{const}^{\prime}+1)\beta^{\prime}{}^{\top}\beta^{\star}\\
\beta^{\prime}{}^{\top}\big(\hat{\beta}-(z_{const}^{\prime}+1)\beta^{\star}\big) & =0
\end{align*}

i.e. $\hat{\beta}-(z_{const}^{\prime}+1)\beta^{\star}\perp\beta^{\prime}$,
so we can say the $\hat{\beta}$ lives on the \textbf{hyper plane}
anchored at $(z_{const}^{\prime}+1)\beta^{\star}$ and normal to $\beta^{\prime}$.
...
Using the abbreviated form, we have 

\[
\kappa(n-\mathrm{df}_{2}\bigl(\kappa\bigr))B_{2,3}(\kappa)=\mathrm{df}_{2,3}(\kappa)\Big(\kappa^{2}B_{1,2}(\kappa)+\sigma^{2}\Big)
\]

or 
\[
\sigma^{2}=\kappa(n-\mathrm{df}_{2}\bigl(\kappa\bigr))\frac{B_{2,3}(\kappa)}{\mathrm{df}_{2,3}(\kappa)}-\kappa^{2}B_{1,2}(\kappa)
\]

Note that using 
\[
E_{gen,DE}=\frac{n\ \big(\kappa^{2}B_{1,2}(\kappa)+\sigma^{2}\big)}{n-\mathrm{df}_{2}\bigl(\kappa\bigr)}-\sigma^{2}
\]

We have alternative condition 
\begin{equation}
n\kappa\frac{B_{2,3}(\kappa)}{\mathrm{df}_{2,3}(\kappa)}=E_{gen,DE}+\sigma^{2}\label{eq:pred_optim_reg_cond}
\end{equation}
...
%
\end{minipage}
\begin{prop}[Optimal regularization for prediction]
On the leading DE level, the stationary point of $E_{gen,DE}$ with
respect to $\kappa$ has the following condition
\begin{align*}
E_{gen,DE}+\sigma^{2} & =n\kappa\frac{B_{2,3}(\kappa)}{\mathrm{df}_{2,3}(\kappa)}\\
\sigma^{2} & =\kappa(n-\mathrm{df}_{2}\bigl(\kappa\bigr))\frac{B_{2,3}(\kappa)}{\mathrm{df}_{2,3}(\kappa)}-\kappa^{2}B_{1,2}(\kappa)
\end{align*}
...
\subsubsection{Regularization that minimizes Accentuation / control error }
\begin{prop}[Optimal regularization for accentuation ]
On the leading DE level the condition of $\lambda$ or $\kappa$
that minimize accentuation error $E_{acc}$ is $\mu_{N}=\mu_{D}$,
which makes $E_{acc}=0,R_{acc}^{2}=1,\text{slope}_{acc}=1$. 

$\exists\lambda\in[0,\infty)$, that satisfy this condition, but solution
is not always unique. The condition also has the following equivalent
forms 
\begin{align*}
E_{gen}+\sigma^{2} & =n\kappa\frac{B_{1,2}(\kappa)}{\mathrm{df}_{1,2}(\kappa)}\\
\sigma^{2} & =\kappa\Big(n-\mathrm{df}_{1}(\kappa)\Big)\frac{B_{1,2}(\kappa)}{\mathrm{df}_{1,2}(\kappa)}\\
\sigma^{2} & =n\lambda(\kappa)\frac{B_{1,2}(\kappa)}{\mathrm{df}_{1,2}(\kappa)}
\end{align*}
\end{prop}
One hypothesize is the common RidgeCV procedure basically optimize
$\kappa$ and $\lambda$ to minimize $E_{gen}$, but not for $E_{acc}$,
what if we explicitly optimize for $E_{acc}$?

It's easy to see if $z=0$ then all three metrics will be perfect!
\begin{align*}
R_{acc}^{2} & =1-z^{2}=1\\
\mathrm{slope}_{acc} & =\frac{1}{1+z}=1\\
\mathrm{MSE}_{acc} & =(\beta^{\star\top}\Sigma\beta^{\star})\big(\frac{z}{z+1}\big)^{2}=0
\end{align*}

To achieve $z=0$, it entails 
\[
\frac{D}{N}-1=0
\]
 i.e. the deterministic equivalent version $\mu_{D}=\mu_{N}$ 

Consider 
\[
(\frac{N}{D}-1)^{2}\approx(\frac{\mu_{N}}{\mu_{D}}-1)^{2}
\]
\begin{align*}
\mu_{N} & \asymp\beta^{*\top}(\Sigma+\kappa I_{d})^{-1}\Sigma\beta^{*}=B_{1,1}(\kappa)\\
\mu_{D} & \asymp\beta^{*\top}(\Sigma+\kappa I_{d})^{-2}\Sigma^{2}\beta^{*}+\frac{1}{n}\mathrm{Tr}\Big[\Sigma(\Sigma+\kappa I_{d})^{-2}\Big]\big(E_{gen}+\sigma^{2}\big)\\
 & =B_{2,2}(\kappa)+\frac{1}{n}\mathrm{df}_{1,2}(\kappa)(E_{gen}+\sigma^{2})
\end{align*}

Can we make it zero i.e. 
\[
\frac{\mu_{N}}{\mu_{D}}-1=0\rightarrow\mu_{N}=\mu_{D}
\]
\begin{align*}
\mu_{D}-\mu_{N} & =B_{2,2}(\kappa)+\frac{1}{n}\mathrm{df}_{1,2}(\kappa)(E_{gen}+\sigma^{2})-B_{1,1}(\kappa)\\
 & =\frac{1}{n}\mathrm{df}_{1,2}(\kappa)(E_{gen}+\sigma^{2})-\kappa B_{1,2}(\kappa)\\
 & =\frac{1}{n}\mathrm{df}_{1,2}(\kappa)(E_{gen}+\sigma^{2}-n\kappa\frac{B_{1,2}(\kappa)}{\mathrm{df}_{1,2}(\kappa)})
\end{align*}

Singular condition is 

\begin{equation}
E_{gen}+\sigma^{2}=n\kappa\frac{B_{1,2}(\kappa)}{\mathrm{df}_{1,2}(\kappa)}\label{eq:acc_optim_reg_cond}
\end{equation}
...
This condition evaluates as
\begin{align*}
\frac{n\ \big(\kappa^{2}B_{1,2}(\kappa)+\sigma^{2}\big)}{n-\mathrm{df}_{2}\bigl(\kappa\bigr)}-n\kappa\frac{B_{1,2}(\kappa)}{\mathrm{df}_{1,2}(\kappa)} & =0\\
\big(\kappa^{2}B_{1,2}(\kappa)+\sigma^{2}\big) & =\kappa(n-\mathrm{df}_{2}\bigl(\kappa\bigr))\frac{B_{1,2}(\kappa)}{\mathrm{df}_{1,2}(\kappa)}\\
\sigma^{2} & =\kappa(n-\mathrm{df}_{2}\bigl(\kappa\bigr))\frac{B_{1,2}(\kappa)}{\mathrm{df}_{1,2}(\kappa)}-\kappa^{2}B_{1,2}(\kappa)\\
\sigma^{2} & =\kappa\frac{B_{1,2}(\kappa)}{\mathrm{df}_{1,2}(\kappa)}\Big(n-\mathrm{df}_{2}\bigl(\kappa\bigr)-\kappa\mathrm{df}_{1,2}(\kappa)\Big)
\end{align*}

Thus using algebraic identity 
\[
\mathrm{df}_{2}\bigl(\kappa\bigr)+\kappa\mathrm{df}_{1,2}(\kappa)=\mathrm{df}_{1}(\kappa)
\]

We get the condition 
\[
\sigma^{2}=\kappa\frac{B_{1,2}(\kappa)}{\mathrm{df}_{1,2}(\kappa)}\Big(n-\mathrm{df}_{1}(\kappa)\Big)
\]

and the self consistent equation gives us identity 
\[
n\lambda=\kappa(n-\mathrm{df}_{1}(\kappa))
\]
...
One can show that this condition has at least one solution across
$\lambda\in[0,+\infty)$
...
\begin{figure}
\begin{centering}
\includegraphics[width=1\textwidth]{/Users/binxuwang/Library/CloudStorage/OneDrive-HarvardUniversity/AccentuationRMTDissociationTheory/Figures/feature_summary_statistics_flow}
\par\end{centering}
\caption{\foreignlanguage{american}{Relation of summary statistics and generalization vs accentuation.}}
\end{figure}
...
\begin{figure}
\begin{centering}
\includegraphics[clip,width=1\linewidth]{/Users/binxuwang/Library/CloudStorage/OneDrive-HarvardUniversity/AccentuationRMTDissociationTheory/Figures/VanHateren_Top500PC_prediction_null_feature_rotation}\\
\includegraphics[width=1\linewidth]{/Users/binxuwang/Library/CloudStorage/OneDrive-HarvardUniversity/AccentuationRMTDissociationTheory/Figures/VanHateren_Top100PC_prediction_null_feature_rotation}
\par\end{centering}
...
\subsection{Effect of noise and Regularization Strength Selection}

\subsubsection{Prediction optimal regularization selection }

Recall that generalization error reads 
\[
E_{gen}\asymp\frac{n}{n-\mathrm{df}_{2}\bigl(\kappa\bigr)}\Big(\kappa^{2}w^{\star}{}^{\top}\bigl(\Sigma_{F}+\kappa(\lambda)I\bigr)^{-2}\Sigma_{F}w^{\star}+S_{\perp}+\sigma^{2}\Big)-\sigma^{2}
\]
...
One can easily take derivative of it against $\kappa$, and find its
stationary point. Basically, we replace $B,\mathrm{df},\kappa$ by
the feature space evaluated version, and $\sigma^{2}$ by $\sigma^{2}+S_{\perp}$.
We get the stationary condition
\[
\kappa(n-\mathrm{df}_{2}\bigl(\kappa\bigr))B_{2,3}(\kappa)=\mathrm{df}_{2,3}(\kappa)\Big(\kappa^{2}B_{1,2}(\kappa)+\sigma^{2}+S_{\perp}\Big)
\]

We can change the representation
\[
\kappa\frac{B_{2,3}(\kappa)}{\mathrm{df}_{2,3}(\kappa)}=\frac{\kappa^{2}B_{1,2}(\kappa)+\sigma^{2}+S_{\perp}}{n-\mathrm{df}_{2}\bigl(\kappa\bigr)}
\]

Using the definition of $E_{gen,DE}$ 
\[
n\kappa\frac{B_{2,3}(\kappa)}{\mathrm{df}_{2,3}(\kappa)}=E_{gen,DE}(\kappa)+\sigma^{2}
\]

which recovers a very similar condition to the pixel space condition 

\subsubsection{Control / accentuation optimal regularization selection }

Similarly the accentuation error's minimum requires 

\[
D=N
\]

or on the leading DE order i.e.
\[
\frac{\mu_{D}}{\mu_{N}}=\frac{\|\tilde{\beta}_{\kappa}\|^{2}+\frac{1}{n}(E_{gen,DE}+\sigma^{2})\mbox{Tr}\bigl[FF^{\top}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-2}\Sigma_{F}\bigr]}{\beta^{\star}{}^{\top}\tilde{\beta}_{\kappa}}=1
\]

equivalently 
\[
E_{gen,DE}(\kappa)+\sigma^{2}=n\frac{\beta^{\star}{}^{\top}\tilde{\beta}_{\kappa}-\|\tilde{\beta}_{\kappa}\|^{2}}{\mbox{Tr}\bigl[FF^{\top}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-2}\Sigma_{F}\bigr]}
\]
...
Algebraically
\begin{align*}
\beta^{\star}{}^{\top}\tilde{\beta}_{\kappa}-\|\tilde{\beta}_{\kappa}\|^{2} & =(\beta^{\star}-\tilde{\beta}_{\kappa})^{\top}\tilde{\beta}_{\kappa}\\
 & =(\beta^{\star}-F^{\top}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-1}F\Sigma\beta^{\star})^{\top}\tilde{\beta}_{\kappa}\\
 & =(F\beta^{\star}-FF^{\top}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-1}F\Sigma\beta^{\star})^{\top}\Sigma_{F}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-1}w^{\star}\\
 & =(F\beta^{\star}-FF^{\top}w^{\star}+FF^{\top}\big(\Sigma_{F}^{\dagger}-\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-1}\big)F\Sigma\beta^{\star})^{\top}\Sigma_{F}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-1}w^{\star}\\
 & =(F\beta^{\star}-FF^{\top}w^{\star}+FF^{\top}\kappa\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-1}\Sigma_{F}^{\dagger}F\Sigma\beta^{\star})^{\top}\Sigma_{F}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-1}w^{\star}\\
 & =((FF^{\top})^{\dagger}F\beta^{\star}-w^{\star}+\kappa\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-1}w^{\star})^{\top}FF^{\top}\Sigma_{F}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-1}w^{\star}\\
 & =(w^{P}-w^{\star})^{\top}FF^{\top}\Sigma_{F}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-1}w^{\star}+\kappa w^{\star^{\top}}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-1}FF^{\top}\Sigma_{F}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-1}w^{\star}\\
 & =(w^{P}-w^{\star})^{\top}FF^{\top}\tilde{w}_{\kappa}+\kappa w^{\star^{\top}}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-1}FF^{\top}\tilde{w}_{\kappa}
\end{align*}

Geometrically with fixed $\beta^{\star}$, 
\[
D=N
\]
\[
\hat{\beta}^{\top}\beta^{\star}=\hat{\beta}^{\top}\hat{\beta}
\]

this satisfies the input space weight geometry, i.e. lying on a sphere 

\subsection{When is control easy to fail while prediction is fine}

For linear feature Ridge regression
\[
z_{acc,DE}\asymp\frac{\|\tilde{\beta}_{\kappa}\|^{2}+\frac{1}{n}(E_{gen}+\sigma^{2})\mbox{Tr}\bigl[FF^{\top}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-2}\Sigma_{F}\bigr]}{\beta^{\star}{}^{\top}\tilde{\beta}_{\kappa}}-1
\]

which can be used to predict all accentuation errors.

Let's first assume $E_{gen}$ is hold fixed, then $\|\tilde{\beta}_{\kappa}\|^{2}$
, $\beta^{\star}{}^{\top}\tilde{\beta}_{\kappa}$ and $\mbox{Tr}\bigl[FF^{\top}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-2}\Sigma_{F}\bigr]$
can all affects control error still. 

Consider the core trace 
\[
\mbox{Tr}\bigl[FF^{\top}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-2}\Sigma_{F}\bigr]
\]

is a geometry quantity i.e. it doesn't rely on any responses. It just
requires the training distribution feature covariance, $\Sigma_{F}$
and the feature outer product $FF^{\top}$

One can write it in the eigenframe of $\Sigma_{F}=\sum_{k}s_{k}\mathbf{v}_{k}\mathbf{v}_{k}^{\top}$,
note here $s_{k}$ is feature space covariance eigen instead of input
space covariance eigen
\begin{align*}
T(\kappa)= & \mbox{Tr}\bigl[FF^{\top}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-2}\Sigma_{F}\bigr]\\
= & \mbox{Tr}\bigl[FF^{\top}\sum_{k=1}^{p}\frac{s_{k}}{(s_{k}+\kappa)^{2}}\mathbf{v}_{k}\mathbf{v}_{k}^{\top}\bigr]\\
= & \sum_{k=1}^{p}\frac{s_{k}}{(s_{k}+\kappa)^{2}}\mathbf{v}_{k}^{\top}FF^{\top}\mathbf{v}_{k}\\
= & \sum_{k=1}^{p}\frac{s_{k}}{(s_{k}+\kappa)^{2}}\|F^{\top}\mathbf{v}_{k}\|^{2}
\end{align*}

Thus the key is the series of magnitude $M_{k}=\|F^{\top}\mathbf{v}_{k}\|^{2}$,
i.e. feature influence on each feature space PC, if input is white. 

We can also interpret it as gradient magnitude when the read out is
aligned with $\mathbf{v}_{k}$. 

Note another way to read this trace is to rewrite it as (assume positive
definiteness, or sum over nonzero $s_{k}$)
\begin{align*}
T(\kappa) & =\sum_{k=1}^{p}\frac{s_{k}}{(s_{k}+\kappa)^{2}}\|F^{\top}\mathbf{v}_{k}\|^{2}\\
 & =\sum_{k=1,s_{k}\neq0}^{p}\frac{s_{k}^{2}}{(s_{k}+\kappa)^{2}}\frac{\|F^{\top}\mathbf{v}_{k}\|^{2}}{s_{k}}\\
 & =\sum_{k=1,s_{k}\neq0}^{p}\frac{s_{k}^{2}}{(s_{k}+\kappa)^{2}}\frac{M_{k}}{s_{k}}
\end{align*}

This last ratio is informative, 
\begin{align*}
\frac{M_{k}}{s_{k}} & =\frac{\|F^{\top}\mathbf{v}_{k}\|^{2}}{\mathbf{v}_{k}^{\top}\Sigma_{F}\mathbf{v}_{k}}\\
 & =\frac{\mathbf{v}_{k}^{\top}FF^{\top}\mathbf{v}_{k}}{\mathbf{v}_{k}^{\top}\Sigma_{F}\mathbf{v}_{k}}\\
 & =\frac{\mathbf{v}_{k}^{\top}FF^{\top}\mathbf{v}_{k}}{\mathbf{v}_{k}^{\top}F\Sigma F^{\top}\mathbf{v}_{k}}
\end{align*}

So the ratio is can be interpreted as the pixel space gradient squared
norm vs the gradient square norm weighted by natural data covariance
of each PC. 

Thus when the backpropagated gradient $\mathbf{g}_{k}=F^{\top}\mathbf{v}_{k}$
has a lot energy on the low variance eigenspaces of $\Sigma$, this
ratio will be large resulting in large 
\[
\frac{M_{k}}{s_{k}}=\frac{\mathbf{g}_{k}^{\top}\mathbf{g}_{k}}{\mathbf{g}_{k}^{\top}\Sigma\mathbf{g}_{k}}
\]

Further the overall trace sum / average this over the profile of $\sum_{k=1,s_{k}\neq0}^{p}\frac{s_{k}^{2}}{(s_{k}+\kappa)^{2}}$,
i.e. the PC modes that is above the effective regularization level
$\kappa$ will contribute the most! 

So we can see => if even the top PCs in the feature space still have
input gradient with a lot of power in low variance directions of $\Sigma$. 

Thus this explains our observations that when models have gradients
which are high frequency / low variance, their control scores are
usually pretty bad. 

\section{Generalization to nonlinear feature }

\subsection{Nonlinear generalization}

We note that in this form it has a direct generalization to nonlinear
feature maps, for feature mapping $\phi:\mathbb{R}^{d}\to\mathbb{R}^{p},\mathbf{x}\mapsto\phi(\mathbf{x})$,
we can evaluate its local linear approximation, i.e. Jacobian, which
is most useful to gradient based accentuation. 
\[
J(\mathbf{x})=\frac{\partial\phi(\mathbf{x})}{\partial\mathbf{x}}\in\mathbb{R}^{p\times d}
\]

Thus a natural generalization of the previous trace form is to evaluate
it against the Jacobian. 
\begin{align*}
T_{\phi}(\mathbf{x},\kappa) & =\sum_{k=1}^{p}\frac{s_{k}}{(s_{k}+\kappa)^{2}}\|\mathbf{v}_{k}^{\top}J(\mathbf{x})\|^{2}\\
 & =\sum_{k=1}^{p}\frac{s_{k}^{2}}{(s_{k}+\kappa)^{2}}\frac{\|\mathbf{v}_{k}^{\top}J(\mathbf{x})\|^{2}}{s_{k}}
\end{align*}

Note that here $\mathbf{g}_{k}(\mathbf{x})=\mathbf{v}_{k}^{\top}J(\mathbf{x})=\nabla_{\mathbf{x}}(\mathbf{v}_{k}^{\top}\phi(\mathbf{x}))$
is the input gradient of the PCs in feature space, exactly the same
meaning as the linear feature case. 

Though now for nonlinear functions, this ratio no longer has the clean
ratio interpretation as $\frac{\mathbf{v}_{k}^{\top}FF^{\top}\mathbf{v}_{k}}{\mathbf{v}_{k}^{\top}F\Sigma F^{\top}\mathbf{v}_{k}}=\frac{\mathbf{g}_{k}^{\top}\mathbf{g}_{k}}{\mathbf{g}_{k}^{\top}\Sigma\mathbf{g}_{k}}$
as in linear case, since the denominator $s_{k}$ cannot generally
be written as the trace form using the local gradient estimate $s_{k}\not\equiv\mathbf{g}_{k}^{\top}(\mathbf{x})\Sigma\mathbf{g}_{k}(\mathbf{x})$, 

But we can still interpret this ratio as the ratio between the gradient
norm from this PC $\mathbf{v}_{k}$ (which is local quantity) vs $s_{k}$,
i.e. evoked variance along this PC by natural data covariance (which
is the global quantity). 

So when models have much larger gradient power (local) than the natural
evoked variance (global), this could be very dangerous, analoguosly. 

\subsection{Empirical validation}

Thus we could think of evaluating this generalized form empirically
\[
\frac{1}{n}(E_{gen}+\sigma^{2})\times\mbox{Tr}\bigl[J(\mathbf{x})J(\mathbf{x})^{\top}\bigl(\Sigma_{F}+\kappa I_{p}\bigr)^{-2}\Sigma_{F}\bigr]=(E_{gen}+\sigma^{2})\times\frac{1}{n}T_{\phi}(\mathbf{x},\kappa)
\]

which is saying the generalization error is scaled by a scalar totally
determined by the input distribution and feature map geometry (i.e.
not related to the teacher or the response)
\begin{itemize}
\item i.e. $(E_{gen}+\sigma^{2})$ is given to us roughly by the prediction
error on held out noisy in-distribution data during cross validation
$E_{gen,val}$. 
\item and then substitute $F$ by a local linear approximation $J$ of $\phi$
around the seed image $\mathbf{x}_{0}$.
\end{itemize}
Then we can evaluate this overall predictor of the control error and
see how it predicts the error metrics of control, like slope, R2, 

One choice we can make is how to evaluate $J$ i.e. local linear approximation.
There are many ways to evaluate it which all have identical results
on linear feature maps. 

The motivation is $J$ as exact Jacobian might be too local, and for
nonlinear mapping and for accentuations that walks in the input space,
this exactly local estimate around the seed can be too local and uninformative. 

So some smoothed estimate across local regions of input can be helpful.
Thus we designed a few kinds of ``smoothed'' local Jacobian and
tested it with correlation with the biological control score. 

\subsubsection{Jacobian estimation}

\paragraph{Numerical consideration}

How to compute the Jacobian? Generally we don't need to evaluate the
full $p\times d$ Jacobian, but just the PC projections of them $ $

On the basic level there are two ways to compute Jacobian 1) backward
auto differentiation to evaluate vector Jacobian product; 2) forward
finite difference / auto differentiation. 

For a lot of PC directions, finite differencing with only forward
pass can be more efficient. 

\paragraph{Schemes of smoothing }

\paragraph{Smooth 
\[
M_{smooth,k}=\|\mathbb{E}_{\mathbf{z}\sim\mathcal{N}(0,I_{d})}[\mathbf{v}_{k}^{\top}J(\mathbf{x}_{0}+\tau\mathbf{z})]\|^{2}
\]
}
...
\begin{align*}
 & \mathbb{E}_{\mathbf{z}\sim\mathcal{N}(0,I_{d})}[\mathbf{v}_{k}^{\top}J(\mathbf{x}_{0}+\tau\mathbf{z})]\\
= & \mathbb{E}_{\mathbf{z}\sim\mathcal{N}(0,I_{d})}[\nabla_{\mathbf{x}}\big(\mathbf{v}_{k}^{\top}\phi(\mathbf{x}_{0}+\tau\mathbf{z})\big)]\\
= & \nabla_{\mathbf{x}}\mathbb{E}_{\mathbf{z}\sim\mathcal{N}(0,I_{d})}[\mathbf{v}_{k}^{\top}\phi(\mathbf{x}_{0}+\tau\mathbf{z})]\\
= & \nabla_{\mathbf{x}}\mathbf{v}_{k}^{\top}\mathbb{E}_{\mathbf{z}\sim\mathcal{N}(0,I_{d})}[\phi(\mathbf{x}_{0}+\tau\mathbf{z})]
\end{align*}

So at population limit, it could be interpreted as the gradient or
Jacobian of locally averaged function. 

Note that this estimator is unbiased estimator for the gradient /
Jacobian of the smooth function, but our goal is the squared norm,
then there are some corrections that could be applied to make it unbiased. 

\paragraph{Neighborhood}

One could consider do the MC averaging outside the squared norm. 
\[
M_{neighbor,k}=\mathbb{E}_{\mathbf{z}\sim\mathcal{N}(0,I_{d})}[\|\mathbf{v}_{k}^{\top}J(\mathbf{x}_{0}+\tau\mathbf{z})\|^{2}]
\]


\paragraph{Variance}

Another direct estimator is to skip the Jacobian directly, and the
following variance will be an estimator too 
\[
M_{var,k}=\frac{1}{\tau^{2}}\mathrm{Var}_{\mathbf{z}\sim\mathcal{N}(0,I_{d})}\big[\mathbf{v}_{k}^{\top}\phi(\mathbf{x}_{0}+\tau\mathbf{z})\big]
\]


\paragraph{Finite-Step}

\[
M_{finite\ step,k}=\frac{1}{\tau^{2}}\mathbb{E}_{\mathbf{z}\sim\mathcal{N}(0,I_{d})}\big[\big(\mathbf{v}_{k}^{\top}\phi(\mathbf{x}_{0}+\tau\mathbf{z})-\mathbf{v}_{k}^{\top}\phi(\mathbf{x}_{0})\big)^{2}\big]
\]


\paragraph{Stein}

Note the smoothed Jacobian can also be written as the following form,
using Stein's identity 
\[
\mathbb{E}_{\mathbf{z}\sim\mathcal{N}(0,I_{d})}[\mathbf{v}_{k}^{\top}J(\mathbf{x}_{0}+\tau\mathbf{z})]=\frac{1}{\tau}\mathbb{E}_{\mathbf{z}\sim\mathcal{N}(0,I_{d})}[\mathbf{v}_{k}^{\top}\phi(\mathbf{x}_{0}+\tau\mathbf{z})\mathbf{z}^{\top}]
\]

and 
\[
M_{Stein,k}=\|\frac{1}{\tau}\mathbb{E}_{\mathbf{z}\sim\mathcal{N}(0,I_{d})}[\mathbf{v}_{k}^{\top}\phi(\mathbf{x}_{0}+\tau\mathbf{z})\mathbf{z}^{\top}]\|^{2}
\]

Usually one needs to add some control variates to reduce the variance
of the estimator e.g. 
\[
\mathbb{E}_{\mathbf{z}\sim\mathcal{N}(0,I_{d})}[\mathbf{v}_{k}^{\top}\big(\frac{\phi(\mathbf{x}_{0}+\tau\mathbf{z})-\phi(\mathbf{x}_{0})}{\tau}\big)\mathbf{z}^{\top}]
\]

So at population limit, this will be the same as smooth gradient. 

Empirically this is very much not stable, and one could use antithetic
noise and increase number of z to make it more stable 
\[
\mathbb{E}_{\mathbf{z}\sim\mathcal{N}(0,I_{d})}[\mathbf{v}_{k}^{\top}\big(\frac{\phi(\mathbf{x}_{0}+\tau\mathbf{z})-\phi(\mathbf{x}_{0}-\tau\mathbf{z})}{2\tau}\big)\mathbf{z}^{\top}]
\]
...
which can be written using df notations below 
\[
\kappa(\lambda)-\lambda=\frac{1}{n}\kappa(\lambda)\mathrm{df}_{1}(\kappa)
\]
