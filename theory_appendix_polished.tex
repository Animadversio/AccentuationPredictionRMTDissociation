...
E_{acc}=\Var[\alpha]\big({\bhat}^\top(\bhat-\bstar)\big)^2,\\
...
\notag\\
&\phantom{:}=\|\bar\beta_\kappa\|^2+V_{\rm pix}(\kappa),\qquad \bar\beta_\kappa:=\Sigma(\Sigma+\kappa I)^{-1}\bstar,
...
\\&\quad+\kappa^2\,\frac{{\bstar}^\top(\Sigma+\kappa I)^{-2}\Sigma\bstar\;\operatorname{Tr}[\Sigma(\Sigma+\kappa I)^{-2}\Sigma]}{n-\mathrm{df}_2}\\
...
&\quad+\frac{\sigma^2}{n}\operatorname{Tr}\big[\uvec_k\uvec_k^\top(\hat\Sigma+\lambda I)^{-1}\hat\Sigma(\hat\Sigma+\lambda I)^{-1}\big].
\end{aligned}
\]
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{figures/tuning_kernels_inset}
...
\mu_N&\asymp{\bstar}^\top\bar\beta_{F,\kappa},\\
...
$q=F\bstar$.  }%\red{(TikZ source: \texttt{figures/feature\_summary\_statistics\_flow\_SigmaF.tex}, also in the validation repository.)}}
...
&\quad+\frac{\operatorname{Tr}\big[A(\Sigma+\kappa I)^{-2}\Sigma\big]\operatorname{Tr}\big[B(\Sigma+\kappa I)^{-2}\Sigma\big]}{n-\mathrm{df}_2(\kappa)}
\Big\},
...
&\quad+\frac{\kappa^2\operatorname{Tr}\big[A(\Sigma+\kappa I)^{-2}\Sigma\big]\operatorname{Tr}\big[B(\Sigma+\kappa I)^{-2}\Sigma\big]}{n-\mathrm{df}_2(\kappa)} .
...
&\quad+\frac{\kappa\kappa'}{n-\mathrm{df}_2(\kappa,\kappa')}\operatorname{Tr}[AG_\Sigma\Sigma G'_\Sigma]\operatorname{Tr}[G'_\Sigma\Sigma G_\Sigma B],\\
\qquad
...
\\
&=1-\gamma\operatorname{tr}\big[\Sigma^2(\Sigma+\kappa I)^{-2}\big]\\
&=1-\frac{\mathrm{df}_2(\kappa)}{n},
\end{align*}
so that
\begin{equation}
