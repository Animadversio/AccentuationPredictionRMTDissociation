...
terms give Eq.~\eqref{eq:app-ratio-mean}.  For the square,
\[
...
which is Eq.~\eqref{eq:app-ratio-second-moment}.
\end{proof}
...
Eq.~\eqref{eq:app-var-ratio-noise}.
\end{proof}

\begin{remark}[What the correction captures and what it misses]
Eq.~\eqref{eq:app-var-ratio-noise} has the right order and the right growth
...
Eq.~\eqref{eq:app-per-pc} (signal and noise fluctuation terms), and evaluates
...
Eq.~\eqref{eq:app-per-pc}, the delta method with $U={\bhat}^\top\bprime$,
...
Eq.~\eqref{eq:feature-fragility-spectrum} gives
\begin{equation}
...
in general, so Eq.~\eqref{eq:nonlinear-local-fragility} is a diagnostic
...
teacher-dependent terms of Eq.~\eqref{eq:app-feature-D-summary} would also be
...
Differentiating Eq.~\eqref{eq:app-kappa-fixed-point} in the form
