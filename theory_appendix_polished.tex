...
\begin{prop}[Restatement of Proposition~\ref{prop:mr-ratio-delta}, second part]
...
and the probability of sign changes of $N$, which the moment expansions
cannot.  It is a surrogate rather than a proved distributional deterministic
equivalent: matching per-coordinate second moments does not match the joint
fluctuations of quadratic forms; it omits off-diagonal, design-induced
covariance; and it does not enforce the exact ridgeless identity $N=D$ that
holds for the minimum-norm interpolator ($\bhat=P_X\bstar$ with $P_X$ a
projector, so $N=D$ for every design).  In practice it agrees with the
leading DE where the latter is accurate and inherits its failures near
$\lambda\to0$.
