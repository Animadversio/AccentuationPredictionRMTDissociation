...
\paragraph{Toolkit.}
We use deterministic equivalents (DEs) in the proportional limit $d/n\to\gamma$ \citep{dobriban2018high,hastie2022surprises}. It allows us to replace certain functions of empirical covariance $\hat\Sigma$ with those of the population covariance $\Sigma$, e.g. $\hat\Sigma(\hat\Sigma+\kappa I)^{-1}\asymp\Sigma(\Sigma+\kappa I)^{-1}$, 
...
This allows us to capture the average effect of finite data $X$, without explicitly taking expectation over random matrices. 
...
E_{gen}\asymp E_{gen,DE}=\frac{n(\kappa^2B_{1,2}+\sigma^2)}{n-\mathrm{df}_2}-\sigma^2,
\quad
N\asymp \mu_N=\underbrace{B_{1,1}}_{\bstar^\top \bar\beta_\kappa},%\mu_N=
\quad
D\asymp \mu_D=\underbrace{B_{2,2}}_{\|\bar\beta_\kappa\|^2}+\underbrace{\frac{\mathrm{df}_{1,2}}{n}\big(E_{gen,DE}+\sigma^2\big)}_{\text{norm inflation }  V}.%
\label{eq:pixel-de}
\end{equation}
\end{prop}
$E_{gen}$ recovers the classic in distribution generalization error  \cite{alex,surya,blake}. 
Let $\bar\beta_\kappa=\Sigma(\Sigma+\kappa I)^{-1}\bstar$, the average student weight (DE) at given regularization. Then $N=B_{1,1}$ and the first term of $D$  are relatively benign and controlled by the teacher weights, since $\|\bstar\|^2\geq B_{1,1} \geq B_{2,2}$ (\ref{eq:}). 
However, the 2nd term of $D$ marked as $V\geq0$ is the key issue. It summarizes how limited data $n$, higher noise ($\sigma^2$), higher generalization error $E_{gen}$ and a spectral summary $\mathrm{df}_{1,2}$ jointly inflate the student norm $D$. $\mathrm{df}_{1,2}$ counts the number of modes near the regularization threshold $\kappa$. 
As a result, increasing $V$ will $D$ causing student to over-predict the variance it induces, leading to $\text{slope}=N/D<1$; it can also inflate $z=D/N-1$ , potentially leading to negative $R^2=1-z^2$. 
Later we will generalize this term $V$ to predict control success of visual neuron (Sec.~\ref{sec:nonlinear}). 

Note that, $V$ is not necessarily bad for control: at a special point where,  $B_{1,1}=B_{2,2}+V$, i.e. this inflation replenishes the norm deflation caused by the shrinkage, thus $N=D$ and control is perfect at leading order (the valley in Fig.~\ref{fig:pixel}\textbf{B}). 
...
Both conditions equate the noisy prediction risk to $n\kappa$ times the average teacher power in a bump like spectral band $B_{i,i}/\mathrm{df}_{i,i}$, where $\mathrm{df}_{i,i}$ specifies the band of averaging. Since $\mathrm{df}_{1,2}\geq\mathrm{df}_{2,3}$, control averages at a slightly higher band. 
...
With $\kappa$, $B^F$, $\mathrm{df}^F$ built from $(\Sigma_F,w^\star)$ and aspect ratio $p/n$,
\begin{equation}\small
...
\label{eq:feature-de}
\end{equation}
where $\tilde\beta_\kappa=F^\top(\Sigma_F+\kappa I)^{-1}F\Sigma\bstar$ is the mean pixel weight and
...
A salient observation is that prediction sees the feature map only through $\Sigma_F$ and $F\Sigma\bstar$: the whole curve $E_{gen,DE}(\kappa)$, hence the cross-validated $\kappa$ and $R^2_{gen,cv}$, is a function of the summary statistics $(\Sigma_F, F\Sigma\bstar, S_\perp, \sigma^2, n)$. Control additionally depends on $\Gram$, through $T_F$ and $\|\tilde\beta_\kappa\|$, and on $F\bstar$, through $N_{DE}$ (Fig.~\ref{fig:feature}A). Thus, for feature regression, \textit{the statistics governing control generically subsume and exceed those governing prediction}; except for special cases, where the two sets coincide (e.g. $\Sigma=I$, so $\Gram=\Sigma_F$,$F\bstar=F\Sigma\bstar$; see \red{App.} for discussion). 
...
The factor $T_F$ is response-independent and has the spectral form
\begin{equation}
