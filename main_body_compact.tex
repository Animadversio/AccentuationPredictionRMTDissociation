...
Peer review evaluates $\bhat$ on the path of a peer $\bprime$, so the roles of $N$ and $D$ are played by the true and predicted gains along the peer's path, $N_{peer}:={\bstar}^\top\bprime$ and $D_{peer}:={\bhat}^\top\bprime$, and every metric is again a function of $z_{peer}:=D_{peer}/N_{peer}-1$, exactly as in Eq.~\ref{eq:acc_metric_ND_depen} (Table~\ref{tab:evaluation-metrics}). Take the peer to be a ridge fit at the same $\kappa$ on independent data. Both fits then concentrate around the same mean weight $\bar\beta_\kappa$ while their noise parts are uncorrelated, so the norm inflation drops out of the cross product: $D_{peer}\asymp\|\bar\beta_\kappa\|^2=B_{2,2}$ and $N_{peer}\asymp\bstar^\top\bar\beta_\kappa=B_{1,1}$ (Prop.~\ref{prop:mr-peer}). Comparing with Eq.~\ref{eq:pixel-de},
\begin{equation}
z_{peer}\asymp\frac{\|\bar\beta_\kappa\|^2}{\bstar^\top\bar\beta_\kappa}-1=\frac{B_{2,2}}{B_{1,1}}-1\in(-1,0],
\qquad
z_{acc}\asymp z_{peer}+\frac{V_{\rm pix}}{\bstar^\top\bar\beta_\kappa}\,,
\label{eq:peer-de}
\end{equation}
that is, \emph{self-review is peer review plus the norm inflation}. Since $B_{1,1}\ge B_{2,2}$ (Eq.~\ref{eq:app-spectral-inequalities}), $z_{peer}\le0$ at leading order: the peer slope $1/(1+z_{peer})=B_{1,1}/B_{2,2}$ is never below one, i.e. the \textit{student under-predicts the teacher on its peer's path}, and $R^2_{peer}=1-z_{peer}^2$ is non-negative and degrades gracefully with noise through the selected $\kappa$. On the self-generated path, $V_{\rm pix}$ adds to $z$ without bound, driving the slope below one and $R^2_{acc}$ negative.
