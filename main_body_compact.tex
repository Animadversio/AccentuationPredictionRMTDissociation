...
Both conditions equate the noisy prediction risk to $n\kappa$ times a band-limited average of the teacher's per-mode power $b_k^2$, but the two bands sit at different places (App.~Fig.~\ref{fig:app-tuning-kernels}). The control kernel $s_k/(s_k+\kappa)^2$ is a bump centered at $s_k=\kappa$, symmetric on a log axis; the prediction kernel $s_k^2/(s_k+\kappa)^3$ is the same bump multiplied by the shrinkage factor $s_k/(s_k+\kappa)$, which shifts it up to $s_k\approx2\kappa$ and suppresses its low-variance side. Each kernel is a $\kappa$-sensitivity: the control condition asks which modes' shrinkage $s_k/(s_k+\kappa)$ changes as $\kappa$ moves, since that sets the mean weight and $N$, whereas prediction stationarity asks which modes' \emph{squared} shrinkage changes, since that sets $E_{gen}$. Consequently $z^{(0)}_{acc,CV}$ compares the teacher power around $2\kappa$ with that around $\kappa$. It vanishes for isotropic data, or for a teacher whose per-mode power $b_k^2$ is constant across modes (Rem.~\ref{rem:app-tuning-remarks}); it is positive, i.e. the cross-validated student over-shoots ($\mathrm{slope}_{acc}<1$), whenever $b_k^2$ decreases toward the low-variance tail, as for a teacher aligned with the leading image modes; and it is negative in the opposite case. For an anisotropic spectrum and a structured teacher the two averages differ generally, and their difference is the control error left at the cross-validated penalty. 
