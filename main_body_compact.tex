...
\paragraph{Toolkit.}
We use deterministic equivalents (DEs) in the proportional limit $d/n\to\gamma$ \citep{dobriban2018high,hastie2022surprises}. It allows us to replace certain functions of the empirical covariance $\hat\Sigma$ with those of population covariance $\Sigma$, e.g. $\hat\Sigma(\hat\Sigma+\kappa I)^{-1}\asymp\Sigma(\Sigma+\kappa I)^{-1}$, 
where finite-sample randomness is absorbed by the effective penalty $\kappa(\lambda)$, the positive root of the self-consistent equation $\kappa-\lambda=\frac{\kappa}{n}\operatorname{Tr}[\Sigma(\Sigma+\kappa I)^{-1}]$. More details about DEs are deferred to App.~\ref{sec:app-de-toolkit}. 
...
\end{equation}
\end{prop}
Both conditions equate the noisy prediction risk to $n\kappa$ times a teacher-weighted spectral average, but they average with different powers of $s_k/(s_k+\kappa)$. For isotropic data the two averages coincide and cross-validation \red{calibrates} control for free. For an anisotropic spectrum they differ whenever the teacher's energy is not uniformly distributed over modes (\red{\ref{close_condition}}). \red{The gap is multiplied by $\kappa\,\mathrm{df}_{1,2}$, i.e. by the number of modes near the regularization edge. In Fig.~\ref{fig:geometry}, Eq.~\ref{eq:pixel-zacc-cv} is the distance along the path between its tangency with the prediction ellipsoid and its crossing of the control sphere.} This answers the question in the title for pixel regression: \emph{control is bad, despite prediction-optimal tuning, when the response is noisy, the spectrum is anisotropic, and many modes sit near $\kappa$}, which is exactly the noisy neuron with natural-image regime. 
...
\begin{figure}[t]
    ...
    % \includegraphics[width=\linewidth]{figures/paper_composite_default_best_figexport.pdf}
...
Now evaluate $\bhat$ on the path of a peer $\bprime$ fitted at the same $\kappa$ with independent noise realization. The two noise realizations are uncorrelated, so the Euclidean noise term that inflates $D$ drops out of the cross products: ${\bhat}^\top\bprime\asymp B_{2,2}$ and ${\bstar}^\top\bprime\asymp B_{1,1}$, and the \red{peer gain} and score are
\begin{equation}
...
\begin{figure}[t]
    ...
    \textbf{A}~Dependency graph: $(\Sigma_F,F\Sigma\bstar,S,\sigma^2,n)$ fix generalization and the cross-validated penalty; control additionally needs $\GramDef$ and $F\bstar$ (draft: to be simplified to $\sim$8 boxes).
    \textbf{B}~Rotating a top-500-PC projector of van Hateren images into the prediction-null spectral tail: $E_{gen}$ and $R^2_{gen}$ are invariant while $R^2_{acc}$ and the slope collapse; DE (lines) matches finite-$n$ Monte Carlo (markers). }
...
\end{prop}
Prediction sees the feature map only through $\Sigma_F$ and $F\Sigma\bstar$: the whole curve $E_{gen,DE}(\kappa)$, hence the cross-validated $\kappa$ and $R^2_{gen}$, is a function of $(\Sigma_F,F\Sigma\bstar,S,\sigma^2,n)$. Control additionally sees $\Gram$, through $T_F$ and $\|\tilde\beta_\kappa\|$, and $F\bstar$, through $N_{DE}$ (Fig.~\ref{fig:feature}A). This is the statistical content of the dissociation: the prediction problem does not identify the control problem.

\begin{prop}[Prediction-preserving gauge]\label{prop:gauge}
For $\Sigma\succ0$ and any orthogonal $Q$ with $Q\Sigma^{1/2}\bstar=\Sigma^{1/2}\bstar$, the map $F\mapsto F\Sigma^{1/2}Q\Sigma^{-1/2}$ preserves $\Sigma_F$ and $F\Sigma\bstar$, hence the training law, $E_{gen,DE}(\kappa)$ and the DE cross-validated penalty, while changing $\Gram$ and $F\bstar$ and therefore control.
\end{prop}
For white data, the accentuation error is also invariant across this gauge group, leading to no dissociation. Variation of accentuation error grows with anisotropy. Rotating within it moves feature energy from high-variance to low-variance pixel directions at no cost to prediction (Fig.~\ref{fig:feature}B): along a continuous path, $E_{gen}$, $R^2_{gen}$ and the selected penalty are exactly constant, while $\operatorname{Tr}(FF^\top)$ grows by four orders of magnitude and $R^2_{acc}$ falls from near one to $-10^{8}$. Random draws from the family span this whole range (\red{App.}), so a population of equally predictive backbones with widely different control is the generic situation, not a construction.

\paragraph{What makes a map fragile.}
The factor $T_F$ is response-independent and has the spectral form (Prop.~\ref{prop:mr-fragility})
\begin{equation}
T_F(\kappa)=\sum_{k:s_k>0}\Big(\frac{s_k}{s_k+\kappa}\Big)^2\frac{\|g_k\|^2}{g_k^\top\Sigma g_k},
\qquad g_k:=F^\top v_k,
\label{eq:TF}
\end{equation}
with $(s_k,v_k)$ the eigenpairs of $\Sigma_F$. It averages, over the feature PCs that survive ridge shrinkage, the ratio between the Euclidean energy of the backpropagated PC gradient and its energy under the natural-image covariance. A map is fragile when its well-resolved feature directions backpropagate into low-variance, high-spatial-frequency pixel directions. This is the linear counterpart of the third observation of Sec.~\ref{sec:phenomena}, and it can be computed from the map and the image statistics alone.
...
% We evaluated ten encoding models at 25 neural recording sites from five subjects. Control-session predictions use a frozen site-specific affine map fitted only on natural-image anchors shared across sessions.
We reproduced the results using published data from \citetPW. 
\textbf{A}~MSE on held-out natural images during the control session, analogous to $E_{\mathrm{val}}\approx E_{gen}+\sigma^2$.
\textbf{B}~Biological control slope of measured over predicted responses for model-generated stimuli. In \textbf{A,B}, bars are mean of 25 neural sites (dots), \red{and error bars denote $\pm2$ subject-level SE}.
\textbf{C}~After removing each site's mean across-model, the factor $V_{\phi}=(E_{\mathrm{val}}/n)T_{\phi}(\kappa)$ computed using Jacobian with neighborhood of scale $16/255$ (\red{cf.Eq.\ref{x}}) is strongly anticorrelated with control slope (Pearson $r=-0.61$). % across 250 site--model pairs 
\textbf{D}~Pearson $-r$ between residual control slope and some predictive factors (held-out MSE, exact trace, exact local $V_{\phi}$, or neighborhood $V_{\phi}$ at four scales); our factor $V,T$ outperforms held-out MSE in predicting control slope; \red{larger values indicate stronger association in the predicted direction}. Markers denote: 10 - all, 9 - exclude Untrained, 8 - exclude adversarial trained (RN50-Robust and CLIPAG), 7 - exclude all three. Stars denote raw two-sided Pearson $p<0.05$.} %Site means are fixed from all ten models.  without multiple-comparison correction
