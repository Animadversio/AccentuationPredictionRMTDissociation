...
We computed these quantities for all 250 site--model pairs (App.~\ref{sec:app-methods-nonlinear}), with $\kappa$ set by each model's cross-validated penalty and feature spectrum, $\E_x$ over the ten seed images, and $E_{val}$ the held-out error of the control session. Because sites differ greatly in controllability, \red{we subtract each site's ten-model mean from the control outcome and correlate this residual with the raw $\log_{10}$ predictors (Pearson $r$; Fig.~\ref{fig:nonlinear-biological-validation}\textbf{C,D})}. Held-out error carries essentially no ordering information (\red{$r=0.02$} with control slope, \red{$-0.04$} with control error, across the nine trained models), whereas exact-local $V_\phi$ reaches \red{$r=-0.58$ and $+0.40$}; the trace $T_\phi$ alone does \red{as well ($-0.61$, $+0.43$)}, so the ordering comes from gradient geometry, not the prediction-error prefactor. Among the seven conventionally trained models the slope association is weaker but same-signed (\red{$r\approx-0.15$ to $-0.17$}): the two adversarially trained models are the strongest controllers and have by far the smallest $T_\phi$. \red{Neighborhood smoothing ($16/255$) keeps more of the ordering without the robust models ($r=-0.25$ vs.\ $-0.05$ for exact-local $V_\phi$ over eight models).} Thus, the gradient-geometry factor of linear feature maps carries over to deep networks, ranking equally predictive encoding models by \textit{in vivo} control where held-out accuracy cannot; the rankings are descriptive (models repeated within sites), not predictions of absolute control error.
