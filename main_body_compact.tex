...
(iii) for ridge regression on a fixed linear feature map, a separation of the summary statistics that determine prediction from those that determine control, and a gauge family of feature maps with identical generalization but arbitrarily different control (Sec.~\ref{sec:feature});
...
\begin{figure}[t]
    ...
    \includegraphics[width=\linewidth,height=0.2\textheight,keepaspectratio]{figures/Figure_LinearToyModel.png}
...
We idealize a visual neuron as a noisy linear teacher, its encoding model as a linear student, and natural image inputs $\x\in\R^d$ as draws from $\mathcal N(0,\Sigma)$, a centered Gaussian distribution with structured covariance $\Sigma=\sum_ks_k\uvec_k\uvec_k^\top$. 
...
\begin{tabular}{@{}llccc@{}}
\toprule
Evaluation & Input covariance & MSE $E_{...}$ & $R^2$ & Slope \\
\midrule
...
\bottomrule
\end{tabular}
\caption{One fitted weight, three evaluation distributions. $N:={\bhat}^\top\bstar$, $D:={\bhat}^\top\bhat$, $\Delta:=\bhat-\bstar$. Pearson correlation is degenerate on a noiseless rank-one path and is omitted.}
...
\end{equation}
\begin{wrapfigure}[24]{r}{0.5\linewidth}
    \vspace{-8pt}
    \centering
    \vspace{-5pt}
    ...
    \label{fig:geometry}
\end{wrapfigure}
Geometrically (Prop.~\ref{prop:mr-iso-geometry}), the equal-$E_{gen}$ sets are ellipsoids around $\bstar$ elongated along low-variance axes, whereas the equal-$z_{acc}$ sets are Euclidean spheres with diameter joining $0$ and $(1+z)\bstar$, and the equal-peer-error sets are hyperplanes normal to $\bprime$ (Fig.~\red{\ref{fig:geometry}}). For control, the sphere diameter directly encodes the shifted error $1+z$, where zero-error students live on the sphere at $z=0$. A student can sit deep inside the prediction ellipsoid, spanning a huge control error sphere, and still lie on the peer hyperplane of a good generator: this is exactly the configuration of Fig.~\ref{fig:phenomena} (bottom), and it can be made arbitrarily extreme by adding weight along a direction with $s_k\to0$ (Exp.~\ref{ex:mr-dissociation}).
...
The question has a geometric reading (Fig.~\ref{fig:geometry}). For a given training set, Eq.~\ref{eq:ridge} traces a path $\bhat(\lambda)$ in weight space from the least-squares or minimum-norm solution at $\lambda\to0$ to the origin at $\lambda\to\infty$. Response noise and the random design scatter the fitted weight in a cloud around its mean, and the cloud is elongated along the low-variance directions of $\Sigma$, because those are the directions the data constrain least. Ideally, cross-validation selects the point where the path is tangent to the smallest reachable prediction ellipsoid; whereas minimal control error asks where the path crosses the $z=0$ sphere $D=N$; in general these two points do not coincide. \red{The analysis below makes this geometric picture quantitative.}
%\red{the two coincide only if the noise cloud has not pushed the path off the sphere in the directions the ellipsoid does not see. The deterministic equivalents below compute where the center of this cloud sits, how much Euclidean spread it carries, and hence the distance between the two points.}


\paragraph{Toolkit.}
We use deterministic equivalents (DEs) in the proportional limit $d/n\to\gamma$ \citep{dobriban2018high,hastie2022surprises}. \red{They replace certain functions of the empirical covariance $\hat\Sigma$ by deterministic functions of the population covariance $\Sigma$}, e.g. $\hat\Sigma(\hat\Sigma+\kappa I)^{-1}\asymp\Sigma(\Sigma+\kappa I)^{-1}$,
where finite-sample randomness is absorbed by the effective penalty $\kappa(\lambda)$, the positive root of the self-consistent equation $\kappa-\lambda=\frac{\kappa}{n}\operatorname{Tr}[\Sigma(\Sigma+\kappa I)^{-1}]$. 
\red{Thus $\kappa$ absorbs the average effect of a finite random design without requiring us to manipulate the realized matrix $X$ explicitly.}
More details about DEs are deferred to App.~\ref{sec:app-de-toolkit}. 
Our results are expressed through the teacher-weighted and unweighted spectral sums
\begin{equation}
...
\red{To expose the part of control that is invisible to prediction, define the mean effective pixel weight and the estimator-induced norm inflation}
\begin{equation}
\red{\bar\beta_\kappa:=\Sigma(\Sigma+\kappa I)^{-1}\bstar,
\qquad
V_{\rm pix}(\kappa):=\frac{\mathrm{df}_{1,2}(\kappa)}{n}\bigl(E_{gen,DE}+\sigma^2\bigr).}
\label{eq:pixel-mean-variance}
\end{equation}
\red{The notation anticipates the feature-space extension: $\bar\beta$ will always denote the mean effective weight seen in pixel space, whereas $V$ will denote the excess Euclidean norm contributed by finite-sample estimation variability.}
...
% \quad
% N\asymp \mu_N=B_{1,1},%\mu_N=
% \quad
% D\asymp \mu_D=B_{2,2}+\underbrace{\frac{\mathrm{df}_{1,2}}{n}\big(E_{gen,DE}+\sigma^2\big)}_{V}.%
% \label{eq:pixel-de}
% \end{equation}
\begin{equation}
\begin{aligned}
E_{gen}&\asymp E_{gen,DE}=\frac{n(\kappa^2B_{1,2}+\sigma^2)}{n-\mathrm{df}_2}-\sigma^2,\\
N&\asymp \mu_N=\underbrace{B_{1,1}}_{\bstar^\top \bar\beta_\kappa},
&
D&\asymp \mu_D=\underbrace{B_{2,2}}_{\|\bar\beta_\kappa\|^2}+\underbrace{\frac{\mathrm{df}_{1,2}}{n}\big(E_{gen,DE}+\sigma^2\big)}_{V_{\rm pix}(\kappa)}.
\end{aligned}
\label{eq:pixel-de}
\end{equation}
\end{prop}
\red{$E_{gen}$ recovers the classical in-distribution generalization error \citep{alex,surya,blake}. The two control moments now have a transparent mean--variance decomposition. The numerator $N=\bstar^\top\bar\beta_\kappa=B_{1,1}$ depends only on the alignment of the teacher with the mean fitted weight. The denominator contains its squared norm, $\|\bar\beta_\kappa\|^2=B_{2,2}$, plus the nonnegative inflation $V_{\rm pix}$. Since each shrinkage factor $s_k/(s_k+\kappa)$ lies in $[0,1]$, $\|\bstar\|^2\geq B_{1,1}\geq B_{2,2}$. By contrast, $V_{\rm pix}$ collects the Euclidean energy of estimation noise: it grows with the noisy prediction risk $E_{gen}+\sigma^2$, is suppressed by sample size $n$, and is amplified by the spectral susceptibility $\mathrm{df}_{1,2}=\sum_k s_k/(s_k+\kappa)^2$, whose kernel is largest around the regularization scale and can accumulate over many weakly constrained modes. Increasing $V_{\rm pix}$ enlarges $D$ without changing $N$, pushing $\mathrm{slope}_{acc}=N/D$ below one and potentially making $R^2_{acc}=1-(D/N-1)^2$ strongly negative. In Sec.~\ref{sec:feature}, the same decomposition survives with a feature-dependent mean weight and norm inflation; in Sec.~\ref{sec:nonlinear}, its variance factor motivates a diagnostic of biological control.}

\red{Importantly, norm inflation is not always harmful. At the special point $B_{1,1}=B_{2,2}+V_{\rm pix}$, it exactly compensates for the norm deflation caused by ridge shrinkage; then $N=D$ and control is perfect at leading order (the valley in Fig.~\ref{fig:pixel}\textbf{B}).}
The condition is summarized below, and it's generally different from the condition for best generalization error which cross validation selects for. 
% \red{
% The three quantities are sums of the same per-eigenmode weight error (Prop.~\ref{prop:mr-per-pc}) with different weights: $E_{gen}$ weights mode $k$ by $s_k$, $D$ weights every mode equally. The last term of $\mu_D$ is the Euclidean energy of estimation noise, $\frac1n(E_{gen}+\sigma^2)$ per effective mode times $\mathrm{df}_{1,2}=\sum_ks_k/(s_k+\kappa)^2$, which counts modes near and below $\kappa$ heavily. It inflates the student's norm without helping it predict, so the student over-shoots along its own path: $z_{acc}>0$, slope $<1$. Ridge also shrinks the signal ($B_{2,2}<B_{1,1}$), which pulls in the opposite direction. Whether the two cancel is a question about $\lambda$. For natural images, whose spectrum decays as a power law over $10^4$ pixel modes, $\mathrm{df}_{1,2}$ counts thousands of weakly constrained high-spatial-frequency modes; the estimation noise that lands there is precisely the high-frequency texture of failed accentuations, and it costs almost nothing in $E_{gen}$.
% }
...
\end{equation}
\end{prop}
Geometrically, along the ridge path (Fig.~\ref{fig:geometry}),  $\kappa_{\rm gen}^{\star}$ is the tangency with the prediction ellipsoid and its crossing of the control sphere reveals $\kappa_{\rm acc}^{\star}$. 
\red{Both conditions equate the noisy prediction risk to $n\kappa$ times a spectrally weighted average of teacher power, but they use different kernels. The control condition weights mode $k$ by $s_k/(s_k+\kappa)^2$, whereas prediction stationarity adds the factor $s_k/(s_k+\kappa)$ and therefore emphasizes better-resolved, higher-variance modes relatively more. Control consequently remains sensitive farther into the low-variance spectral tail.}
For isotropic data the two averages coincide and cross-validation nullifies control error for free; similarly when the teacher's energy is uniformly distributed over modes  (\red{\ref{close_condition}}). For an anisotropic spectrum and a structured teacher, they differ generally, and their difference is proportional to the control error $z$ at cross validated penalty $\kappa_{\rm gen}^{\star}$. 
...
Our DE formula generally well tracks the Monte Carlo estimate of the error (Fig.\ref{suppfig:pixl-MC}), where deviation from leading DE occurs only at very low noise or low regularization cases. 
...
\begin{figure}[t]
    ...
    This picture is validated by the numerical MC estimated landscapes in \red{Fig.~\ref{}}. 
    ...
    \textbf{F}~Along the prediction-optimal path, $R^2_{gen}$ remains high while $R^2_{acc}$ becomes strongly negative; empirical RidgeCV exhibits the same dissociation.}
    \vspace{-15pt}
    \label{fig:pixel}
\end{figure}
...
Peer review evaluate $\bhat$ on the path of a peer $\bprime$. Here we assume the peer is fitted at the same $\kappa$ with independent noise realization. The two noise realizations are uncorrelated, so the Euclidean noise term that inflates $D$ drops out of the cross products: ${\bhat}^\top\bprime\asymp B_{2,2}$ and ${\bstar}^\top\bprime\asymp B_{1,1}$, since $B_{1,1}\geq B_{2,2}$ (\red{Eq.~\ref{eq:app}}), the peer slope and score are
\begin{equation}
%G_{peer}=\frac{{\bhat}^\top\bprime}{{\bstar}^\top\bprime}\asymp\frac{B_{2,2}}{B_{1,1}}\in(0,1]
\text{slope}_{peer}=\frac{{\bstar}^\top\bprime}{{\bhat}^\top\bprime}\asymp\frac{B_{1,1}}{B_{2,2}}\in[1,\infty),
\qquad
R^{2,(0)}_{peer}=1-\Big(1-\frac{B_{2,2}}{B_{1,1}}\Big)^2\in(0,1]
\label{eq:peer-de}
\end{equation}
(Prop.~\ref{prop:mr-peer}). 
Note, on the leading order, the peer review slope is never less than $1$, i.e., generally, the \textit{student is under-predicting the teacher on its peer's path}; the peer review $R^2$ is non-negative, and degrades gracefully with noise, through CV selected regularization $\kappa_{cv}$. 
In contrast, for self generated path, the joint effect of finite data, noise, and generalization error ($\frac{\mathrm{df}_{1,2}}{n}\big(E_{gen,DE}+\sigma^2\big)$ in Eq.~\ref{eq:pixel-de}) inflates the student norm $D=\|\bhat\|^2$, causing over-prediction of teacher ($\text{slope}_{acc}=N/D<1$), and arbitrarily large $z_{acc}=D/N-1$ and negative $R^2$. 
The sign differences in neural data confirm this leading order picture: in \citetPW, the peer review score between pairs of models, have $R^2=0.04,[-0.06,0.10]$, $slope=1.35,[1.14,1.59]$ ($n=90$); while accentuation score (eval on self generated path) have $R^2=-4.17,[-4.59,-3.51]$, $slope=0.38,[0.33,0.42]$ (median, [IQR] $n=10$). % stats test between the two \todo
Note, in simplistic pixel space peer review, the slope and $R^2$ are symmetric between models ($A\leftarrow B$ or $B\leftarrow A$). In practice, since students regress onto different features, the review score becomes asymmetric (Sec.~\ref{sec:feature}). 
...
\begin{figure}[t]
    ...
    \textbf{A}~Dependency graph: $(\Sigma_F,F\Sigma\bstar,S,\sigma^2,n)$ fix generalization and the cross-validated penalty; control additionally needs $\GramDef$ and $F\bstar$. 
    \textbf{B-D}~For van Hateren images, rotating the top-500-PC projector feature into the spectral tail: $E_{gen}$ and $R^2_{gen}$ are invariant while $R^2_{acc}$ and the slope collapse; DE (lines) matches finite-$n$ Monte Carlo (markers). }
...
\red{With $\kappa$, $B^F$, and $\mathrm{df}^F$ built from $(\Sigma_F,w^\star)$ and aspect ratio $p/n$, define}
\begin{equation}
\red{\begin{aligned}
\bar\beta_{F,\kappa}&:=F^\top(\Sigma_F+\kappa I)^{-1}F\Sigma\bstar,\\
T_F(\kappa)&:=\operatorname{Tr}[\Gram(\Sigma_F+\kappa I)^{-2}\Sigma_F],
&
...
\end{equation}
\end{prop}
% A salient observation is: prediction sees the feature map only through $\Sigma_F$ and $F\Sigma\bstar$, so the whole curve $E_{gen,DE}(\kappa)$, hence the cross-validated $\kappa$ and $R^2_{gen,cv}$, is a function of the sumary statistics $(\Sigma_F,F\Sigma\bstar,S,\sigma^2,n)$. Control additionally sees $\Gram$, (through $T_F$ and $\|\tilde\beta_\kappa\|$), and $F\bstar$, (through $N_{DE}$) (Fig.~\ref{fig:feature}A). Thus, for feature regression, 
%This is the statistical content of the dissociation: the prediction problem does not identify the control problem.
\red{This is the same mean--variance structure as pixel ridge, now with a feature-dependent mean effective pixel weight $\bar\beta_{F,\kappa}$ and norm inflation $V_F$. The key difference is what determines them. Prediction sees the feature map only through $\Sigma_F$ and $F\Sigma\bstar$: the whole curve $E_{gen,DE}(\kappa)$, hence the cross-validated $\kappa$ and $R^2_{gen,cv}$, is a function of $(\Sigma_F,F\Sigma\bstar,S_\perp,\sigma^2,n)$. Control additionally depends on $\Gram$, through $T_F$ and $\|\bar\beta_{F,\kappa}\|$, and on $F\bstar$, through $N_{DE}$ (Fig.~\ref{fig:feature}A). Thus the statistics governing control generically exceed those governing prediction. They coincide only in special cases---for example, if $\Sigma=I$, then $\Gram=\Sigma_F$ and $F\bstar=F\Sigma\bstar$ (see Appendix for discussion).}
...
\red{The map-dependent part of $V_F$ is the response-independent factor $T_F$, which has the spectral form}
\begin{equation}
T_F(\kappa):=\operatorname{Tr}[FF^\top(\Sigma_F+\kappa I)^{-2}\Sigma_F]=\sum_{k:s_k>0}\Big(\frac{s_k}{s_k+\kappa}\Big)^2\frac{\|g_k\|^2}{g_k^\top\Sigma g_k},
\qquad g_k:=F^\top v_k,
\label{eq:TF}
\end{equation}
...
Visual neuronal responses are known to be highly noisy \red{\cite{?}} meeting (i); 
...
Control slope was mean-subtracted within site using all ten models; predictors remained raw. Markers denote 10/9/8/7-model subsets (all / no untrained AlexNet / no robust models (CLIPAG,RN50-robust) / neither), and stars indicate raw two-sided Pearson $p<0.05$.} %without multiple-comparison correction
