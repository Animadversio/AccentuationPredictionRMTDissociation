...
For \textit{an anisotropic spectrum and a structured teacher} the two averages differ generally, and their difference is the control error left at the cross-validated penalty. Notably, when $b_k^2$ decreases toward the low-variance tail, as for a teacher aligned with the high variance image modes, $z^{(0)}_{acc,CV}>0$ is positive, and the cross-validated student over-predicts ($\mathrm{slope}_{acc}=1/(1+z)<1$). 
